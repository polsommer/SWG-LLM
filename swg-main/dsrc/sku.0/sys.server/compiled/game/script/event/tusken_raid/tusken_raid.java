package script.event.tusken_raid;
...
public class tusken_raid extends base_script
{
    private static final String MOB_TEMPLATE = "tusken_raider";
    private static final int TOTAL_WAVES = 3;
    private static final float WAVE_DELAY = 60.0f;

    public int OnAttach(obj_id self) throws InterruptedException
    {
        debugServerConsoleMsg(self, "[TUSKEN RAID] Invasion initialized.");
    ...
    public int startWave(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (wave >= TOTAL_WAVES)
        {
            debugServerConsoleMsg(self, "[TUSKEN RAID] All waves complete.");
            return SCRIPT_CONTINUE;
        }

        debugServerConsoleMsg(self, "[TUSKEN RAID] Spawning wave " + (wave + 1));
