...
public class base_newbie_npc_spawner extends script.base_script
{
	...
	public void spawnCreatures(obj_id self) throws InterruptedException
	{
		...
		while (count < maxPop)
		{
			...
			String professionFantasy = spawning.inferProfessionFantasyFromCreatureType(whatToSpawn);
			boolean deterministicAppearance = true;
			if (hasObjVar(self, "appearance.seedByNpcId"))
			{
				deterministicAppearance = getIntObjVar(self, "appearance.seedByNpcId") == 1;
			}
			spawning.applyGeneratedNpcAppearance(self, spawned, professionFantasy, deterministicAppearance);
			attachScript(spawned, "creature_spawner.death_msg");
			if (shouldAttachPlayerSimAi(whatToSpawn))
			{
				attachScript(spawned, "library.player_sim_ai");
			}
	...
	public boolean shouldAttachPlayerSimAi(String template) throws InterruptedException
	{
		if (template == null)
		{
			return false;
		}
		return template.equals("flail_cutthroat") || template.equals("scurrier") || template.equals("wolf_kima");
	}
