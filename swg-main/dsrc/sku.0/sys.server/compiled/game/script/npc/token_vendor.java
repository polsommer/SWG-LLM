...
public class token_vendor extends script.base_script
{
    ...
    public int handleItemSelect(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        int tokenCosts[] = new int[tokenmanager.HEROIC_TOKENS.length + 1];
        ...
        int[] intendedPayment = new int[tokenmanager.HEROIC_TOKENS.length];
        ...
        {
            tokenCosts[tokenmanager.HEROIC_TOKENS.length] = d.getInt("any_token");
            int totalCost = tokenCosts[tokenmanager.HEROIC_TOKENS.length];
            ...
            if (totalCost == 0)
            {
                tokenCosts[tokenmanager.HEROIC_TOKENS.length] = 1;
            }
    ...
    public int handlePaymentUpdate(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (tokensOwed[idx] < 1 && tokensOwed[tokenmanager.HEROIC_TOKENS.length] < 1)
        {
        ...
        else if (tokensOwed[tokenmanager.HEROIC_TOKENS.length] > 0)
        {
            tokensOwed[tokenmanager.HEROIC_TOKENS.length]--;
            intendedPayment[idx]++;
        }
        ...
        for (int i1 : tokensOwed) {
            ...
            if (tokensPaidInFull == tokenmanager.HEROIC_TOKENS.length + 1) {
    ...
    {
        int[] totalTokens = new int[tokenmanager.HEROIC_TOKENS.length + 1];
        String[] inventoryTokens = new String[tokenmanager.HEROIC_TOKENS.length];
        ...
        for (obj_id inventoryContent : inventoryContents) {
            ...
            if (itemName != null && !itemName.equals("")) {
                for (int j = 0; j < tokenmanager.HEROIC_TOKENS.length; j++) {
                    if (itemName.equals(tokenmanager.HEROIC_TOKENS[j])) {
                ...
                if (!foundTokenHolderBox && itemName.equals("item_heroic_token_box_01_01")) {
                    ...
                    if (hasObjVar(inventoryContent, "item.set.tokens_held")) {
                        ...
                        for (int k = 0; k < tokenmanager.HEROIC_TOKENS.length; k++) {
        ...
        for (int i = 0; i < tokenmanager.HEROIC_TOKENS.length; i++)
        {
            inventoryTokens[i] = "[" + totalTokens[i] + "] " + getString(new string_id("static_item_n", tokenmanager.HEROIC_TOKENS[i]));
        }
    ...
    public void processItemPurchase(obj_id player) throws InterruptedException
    {
        ...
        for (obj_id inventoryContent : inventoryContents) {
            ...
            if (itemName != null && !itemName.equals("")) {
                for (int j = 0; j < tokenmanager.HEROIC_TOKENS.length; j++) {
                    if (itemName.equals(tokenmanager.HEROIC_TOKENS[j]) && intendedPayment[j] > 0) {
                        if (getCount(inventoryContent) > 1) {
    ...
    public boolean verifyTokens(obj_id player, int[] intendedPayment) throws InterruptedException
    {
        ...
        int tokensNeeded = 0;
        int tokensIGot = 0;
        int[] payment = new int[tokenmanager.HEROIC_TOKENS.length];
        ...
        for (obj_id inventoryContent : inventoryContents) {
            ...
            if (itemName != null && !itemName.equals("")) {
                for (int j = 0; j < tokenmanager.HEROIC_TOKENS.length; j++) {
                    if (itemName.equals(tokenmanager.HEROIC_TOKENS[j]) && payment[j] > 0) {
                        if (getCount(inventoryContent) > 1) {
    ...
    public String getTokensOwedPrompt(obj_id player) throws InterruptedException
    {
        ...
        for (int i = 0; i < tokenmanager.HEROIC_TOKENS.length; i++)
        {
            if (tokensOwed[i] > 0)
            {
                prompt += "[" + tokensOwed[i] + "] " + getString(new string_id("static_item_n", tokenmanager.HEROIC_TOKENS[i])) + "\n";
            }
        }
        if (tokensOwed[tokenmanager.HEROIC_TOKENS.length] > 0)
        {
            prompt += "[" + tokensOwed[tokenmanager.HEROIC_TOKENS.length] + "] " + getString(new string_id("set_bonus", "vendor_sui_any_token"));
        }
        return prompt;
    }
