...
public class cmd extends script.base_script
{
    ...
    public int cmdEconomyOps(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        dictionary parsed = gm.parseTarget(params, target, "ECONOMY");
        if (parsed == null)
        {
            return SCRIPT_CONTINUE;
        }
        ...
        obj_id context = self;
        obj_id parsedTarget = parsed.getObjId("oid");
        if (isIdValid(parsedTarget))
        {
            context = parsedTarget;
        }
        if (!isIdValid(context) || !exists(context))
        {
            context = self;
        }

        obj_id sharedState = vendor_lib.getEconomyStateObject(context);
        if (!isIdValid(sharedState) || sharedState == obj_id.NULL_ID || !exists(sharedState))
        {
            sendSystemMessageTestingOnly(self, "[ECONOMY] No shared economy state found. Use -target on a bazaar/vendor terminal.");
            return SCRIPT_CONTINUE;
        }

        int score = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SCORE) ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SCORE) : 0;
        String mode = economy_stabilizer.getActiveEconomyMode(context, sharedState);
        int online = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_ONLINE) ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_ONLINE) : getNumPlayers();
        int listingVolume = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_LISTINGS) ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_LISTINGS) : 0;
        int created = hasObjVar(sharedState, vendor_lib.SHORTAGE_REWARD_ROOT + ".kpi.listingsCreated") ? Math.max(0, getIntObjVar(sharedState, vendor_lib.SHORTAGE_REWARD_ROOT + ".kpi.listingsCreated")) : 0;
        int sold = hasObjVar(sharedState, vendor_lib.SHORTAGE_REWARD_ROOT + ".kpi.listingsSold") ? Math.max(0, getIntObjVar(sharedState, vendor_lib.SHORTAGE_REWARD_ROOT + ".kpi.listingsSold")) : 0;
        String sellThrough = created > 0 ? (Math.round((sold * 1000.0f) / (float)created) / 10.0f) + "%" : "n/a";
        float feeMultiplier = economy_stabilizer.getAuctionFeeMultiplier(context, sharedState);
        float feeRounded = Math.round(feeMultiplier * 1000.0f) / 1000.0f;

        sendSystemMessageTestingOnly(self, "[ECONOMY] score=" + score + " mode=" + mode + " online=" + online + " listings=" + listingVolume + " sellThrough=" + sellThrough + " feeMult=" + feeRounded);
        sendSystemMessageTestingOnly(self, "[ECONOMY] shortage " + buildShortageSummary(sharedState));
        sendSystemMessageTestingOnly(self, "[ECONOMY] recent " + buildRecentStabilizationSummary(sharedState));
        return SCRIPT_CONTINUE;
    }
    ...
    private String buildShortageSummary(obj_id sharedState) throws InterruptedException
    {
        String[] categories = economy_stabilizer.SUPPLY_CATEGORIES;
        String summary = "";
        for (int i = 0; i < categories.length; i++)
        {
            String category = categories[i];
            int flag = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".flag") ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".flag") : 0;
            int essential = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".essential") ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".essential") : 0;
            int live = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".live") ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".live") : 0;
            int target = hasObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".target") ? getIntObjVar(sharedState, economy_stabilizer.OBJVAR_ECONOMY_SHORTAGE_ROOT + "." + category + ".target") : 0;
            String marker = flag == 1 ? (essential == 1 ? "!" : "*") : "-";
            if (summary.length() > 0)
            {
                summary += " | ";
            }
            summary += category + marker + ":" + live + "/" + target;
        }
        return summary;
    }
    private String buildRecentStabilizationSummary(obj_id sharedState) throws InterruptedException
    {
        String historyPath = vendor_lib.SHORTAGE_REWARD_ROOT + ".recent.actions";
        String[] history = hasObjVar(sharedState, historyPath) ? getStringArrayObjVar(sharedState, historyPath) : null;
        if (history == null || history.length < 1)
        {
            return "none";
        }

        int max = Math.min(3, history.length);
        String out = "";
