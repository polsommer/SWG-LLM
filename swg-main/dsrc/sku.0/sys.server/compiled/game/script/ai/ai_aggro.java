...
public class ai_aggro extends script.base_script
{
    ...
    public static int getAggroStatus(obj_id target) throws InterruptedException
    {
        ...
        else if (isInvulnerable(self) && !ai_lib.isAttackable(self))
        {
            LOGC(aiLoggingEnabled(self), "debug_ai", "ai_aggro::getAggroStatus() self(" + self + ":" + getName(self) + ") target(" + target + ":" + getName(target) + ") I AM INVULNERABLE");
            debugSpeakMsgc(aiLoggingEnabled(self), self, "AggroCheck(" + target + ") I AM INVULNERABLE");
        }
