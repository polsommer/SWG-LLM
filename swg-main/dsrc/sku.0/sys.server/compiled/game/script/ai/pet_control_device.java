...
public class pet_control_device extends script.base_script
{
    ...
    private void clearPetCreationPending(obj_id player) throws InterruptedException
    {
        if (isIdValid(player) && utils.hasScriptVar(player, "petCreationPending"))
        {
            utils.removeScriptVar(player, "petCreationPending");
        }
    }
    private obj_id getPcdOwner(obj_id pcd) throws InterruptedException
    {
        ...
        obj_id owner = getContainedBy(datapad);
        if (!isIdValid(owner))
        {
            return obj_id.NULL_ID;
        }
    ...
    public int delayCreatePet(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        {
            clearPetCreationPending(getPcdOwner(self));
            return SCRIPT_CONTINUE;
        }
        ...
        {
            utils.removeScriptVar(player, CALLED_FOR_PET);
            clearPetCreationPending(player);
            return SCRIPT_CONTINUE;
        }
        if (!isGod(player))
        {
    ...
    public int OnDestroy(obj_id self) throws InterruptedException
    {
        ...
        clearPetCreationPending(master);
