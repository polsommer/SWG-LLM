...
public class terminal_space_tcg extends script.terminal.base.base_terminal
{
    public terminal_space_tcg()
{
}
...
{
    // Define default coordinates and scene if no suitable city regions are found
    float defaultSpaceX = 1000.0f;
    float defaultSpaceY = 2000.0f;
    float defaultSpaceZ = 3000.0f;
    String defaultSpaceScene = "space_yavin4";

    String defaultGroundScene = "tatooine";
    float defaultGroundX = 10.0f;
    float defaultGroundY = 20.0f;
    float defaultGroundZ = 30.0f;
    String defaultPointName = "Mos Eisley Spaceport";

    // Attempt to find suitable city regions
    ...
    if (rgnCities != null && rgnCities.length > 0) {
        // Loop through city regions to find a valid teleport location
        for (region rgnTest : rgnCities) {
            String strName = rgnTest.getName();
            if (strName.startsWith("@")) {
                string_id strTest = utils.unpackString(strName);
                strName = strTest.getAsciiId();
                dctTeleportInfo = dataTableGetRow("datatables/space_zones/launch_locations.iff", strName);
    ...
    // If no suitable teleport info is found, use default coordinates and scenes
    if (dctTeleportInfo == null) {
        dctTeleportInfo = new dictionary();
    ...
    // Set script variables for space and ground locations
