...
public class gcw_spawner extends script.base_script
{
    ...
    public int beginSpawn(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        else if (spawned.length != numberOfCreaturesToSpawn)
        {
            obj_id[] resizedSpawned = new obj_id[numberOfCreaturesToSpawn];
            int copyCount = Math.min(spawned.length, numberOfCreaturesToSpawn);
            for (int i = 0; i < copyCount; ++i)
            {
                resizedSpawned[i] = spawned[i];
            }
            spawned = resizedSpawned;
        }
