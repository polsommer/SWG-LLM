...
public class aurilia_buff_vendor extends script.base_script
{
    ...
    public String createPureConcatenatedJoy(obj_id self) throws InterruptedException
    {
        ...
        if (tokenCost.length > tokenmanager.HEROIC_TOKENS.length)
        {
            tokenArrayLength = tokenmanager.HEROIC_TOKENS.length;
        }
        ...
        for (int i = 0; i < tokenArrayLength; i++)
        {
            if (tokenCost[i] > 0)
            {
                pureConcatenatedJoy += "[" + tokenCost[i] + "] " + getString(new string_id("static_item_n", tokenmanager.HEROIC_TOKENS[i])) + "\n";
            }
        }
        if (creditCost > 0)
        {
            ...
            for (obj_id inventoryContent : inventoryContents) {
                ...
                if (itemName != null && !itemName.equals("")) {
                    for (int j = 0; j < tokenmanager.HEROIC_TOKENS.length; j++) {
                        if (itemName.equals(tokenmanager.HEROIC_TOKENS[j]) && tokenCostInThisFunction[j] > 0) {
    ...
    public void processItemPurchase(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        for (obj_id inventoryContent : inventoryContents) {
            ...
            if (itemName != null && !itemName.equals("")) {
                for (int j = 0; j < tokenmanager.HEROIC_TOKENS.length; j++) {
                    if (itemName.equals(tokenmanager.HEROIC_TOKENS[j]) && tokenCostForReals[j] > 0) {
                        if (getCount(inventoryContent) > 1) {
