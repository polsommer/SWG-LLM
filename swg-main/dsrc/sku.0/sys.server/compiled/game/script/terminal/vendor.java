...
public class vendor extends script.terminal.base.base_terminal
{
    ...
    public int handleNpcVendorPurchase(obj_id self, dictionary params) throws InterruptedException
    {
        vendor_lib.handleNpcVendorPurchase(self, params);
        return SCRIPT_CONTINUE;
    }
    public int handleNpcVendorListing(obj_id self, dictionary params) throws InterruptedException
    {
        vendor_lib.handleNpcVendorListing(self, params);
        return SCRIPT_CONTINUE;
    }
    ...
    public int OnMaintenanceLoop(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        int preEconomyCost = cost;
        cost = vendor_lib.applyEconomyMaintenanceSinkTuning(self, cost);
        CustomerServiceLog("vendor", "Vendor calculating maintenance fees.  Vendor " + self + ", economy mode tuning adjusted loop maintenance from " + preEconomyCost + " to " + cost);
        if (cost < 0)
        {
            cost = 0;
        }
        ...
        if (condition != max_condition)
        {
            ...
            int preEconomyRepairCost = per_point_cost;
            per_point_cost = vendor_lib.applyEconomyMaintenanceSinkTuning(self, per_point_cost);
            CustomerServiceLog("vendor", "Vendor calculating maintenance fees.  Vendor " + self + ", economy mode tuning adjusted repair point cost from " + preEconomyRepairCost + " to " + per_point_cost + ".");
