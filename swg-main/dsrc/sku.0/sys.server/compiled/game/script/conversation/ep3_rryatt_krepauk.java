...
public class ep3_rryatt_krepauk extends script.base_script
{
    ...
    public int OnStartNpcConversation(obj_id self, obj_id player) throws InterruptedException
    {
        ...
        if (ep3_rryatt_krepauk_condition_finishedFeralWookieeHunt(player, npc))
        {
            HashSet theSet = new HashSet();
            obj_id pInv = utils.getInventoryContainer(player);
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series8_yoda_backpack", pInv));
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series8_yoda_house_deed", pInv));
        ...
        if (ep3_rryatt_krepauk_condition_finishedExJediHunt(player, npc))
        {
            HashSet theSet = new HashSet();
            obj_id pInv = utils.getInventoryContainer(player);
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series7_atpt_walker", pInv));
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series6_jedi_council_diorama", pInv));
        ...
        if (ep3_rryatt_krepauk_condition_finishedWallugaHunt(player, npc))
        {
            HashSet theSet = new HashSet();
            obj_id pInv = utils.getInventoryContainer(player);
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series5_klorri_clan_shield", pInv));
            theSet.add(static_item.createNewItemFunction("item_tcg_loot_reward_series3_battle_droid_vendor", pInv));
