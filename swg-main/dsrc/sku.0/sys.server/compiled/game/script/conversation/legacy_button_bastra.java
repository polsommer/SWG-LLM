...
public class legacy_button_bastra extends script.base_script
{
    ...
    {
        boolean valarianComplete = legacy_button_bastra_action_completeQuestIfTasksFinished(player, "quest/legacy_button_reb_valarian") || legacy_button_bastra_action_completeQuestIfTasksFinished(player, "quest/legacy_button_reb_valarian_v2");
        return valarianComplete && !groundquests.isQuestActiveOrComplete(player, "quest/legacy_button_reb_darklighter");
    }
    public boolean legacy_button_bastra_action_completeQuestIfTasksFinished(obj_id player, String questName) throws InterruptedException
    {
        ...
        int questId = groundquests.getQuestIdFromString(questName);
        boolean hasAnyCompletedTask = false;
        for (int i = 0; i < groundquests.maxTasksPerQuest; ++i)
        {
            if (questIsTaskActive(questId, i, player))
            {
                return false;
            }
            if (questIsTaskComplete(questId, i, player))
            {
