    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item) throws InterruptedException
    {
        ...
        if (item == menu_info_types.ITEM_USE)
        {
            if (hasObjVar(self, "grant_xp_percent"))
            {
                ...
                if (playerLevel >= 130)
                {
                    CustomerServiceLog("tcg", "TCG Item(" + self + ")used by player: (" + player + ")" + getFirstName(player) + " Player Level: " + playerLevel + ". Since player is at level cap, this player receives a random collection object.");
