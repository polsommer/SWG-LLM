...
{
    public static final String MUSTAFAR_PLANET = "mustafar";
    public static final String MUSTAFAR_SPAWN_HEARTBEAT_MESSAGE = "handleMustafarSpawnHeartbeat";
    public static final String MUSTAFAR_SPAWN_HEARTBEAT_SCRIPT_VAR = "spawning.mustafarHeartbeatScheduled";
    public static final int MUSTAFAR_SPAWN_HEARTBEAT_ATTEMPTS = 3;
    public static final String MUSTAFAR_FALLBACK_LAIR_TEMPLATE = "object/tangible/lair/npc_lair.iff";
    public spawn_player()
    {
    }
    public int OnInitialize(obj_id self) throws InterruptedException
    {
        ...
        scheduleMustafarSpawnHeartbeat(self, rand(SPAWN_PLAYER_DELAY_MIN, SPAWN_PLAYER_DELAY_MAX));
    ...
    public void scheduleMustafarSpawnHeartbeat(obj_id self, float delay) throws InterruptedException
    {
        if (!isIdValid(self))
        {
            return;
        ...
        if (locCurrentLocation == null || locCurrentLocation.area == null || !locCurrentLocation.area.equals(MUSTAFAR_PLANET))
        {
            utils.removeScriptVar(self, MUSTAFAR_SPAWN_HEARTBEAT_SCRIPT_VAR);
            return;
        }
        if (utils.hasScriptVar(self, MUSTAFAR_SPAWN_HEARTBEAT_SCRIPT_VAR))
        {
            return;
        }
        utils.setScriptVar(self, MUSTAFAR_SPAWN_HEARTBEAT_SCRIPT_VAR, 1);
        messageTo(self, MUSTAFAR_SPAWN_HEARTBEAT_MESSAGE, null, delay, false);
    }
    public int handleMustafarSpawnHeartbeat(obj_id self, dictionary params) throws InterruptedException
    {
        utils.removeScriptVar(self, MUSTAFAR_SPAWN_HEARTBEAT_SCRIPT_VAR);
        if (!isIdValid(self))
        {
        ...
        for (int i = 0; i < MUSTAFAR_SPAWN_HEARTBEAT_ATTEMPTS; i++)
        {
            messageTo(self, "mustafarSpawnTrigger", null, i * rand(4, 8), false);
        }
        scheduleMustafarSpawnHeartbeat(self, rand(SPAWN_PLAYER_DELAY_MIN, SPAWN_PLAYER_DELAY_MAX));
        return SCRIPT_CONTINUE;
    }
    public int mustafarSpawnTrigger(obj_id self, dictionary params) throws InterruptedException
    {
        if (!isIdValid(self))
        {
            return SCRIPT_CONTINUE;
        ...
        {
            doMustafarFallbackSpawn(self, locCurrentLocation, intLevel);
            return SCRIPT_CONTINUE;
        }
        ...
        dctSpawnParams.put("boolTheatersAllowed", false);
        dctSpawnParams.put("boolAllowChallengingSpawns", true);
        doSpawnEvent(dctSpawnParams);
        return SCRIPT_CONTINUE;
    }
    public void doMustafarFallbackSpawn(obj_id self, location locCurrentLocation, int intLevel) throws InterruptedException
    {
        location locSpawnLocation = locations.getGoodLocationAroundLocation(locCurrentLocation, 25.0f, 25.0f, rand(120, 220), rand(120, 220), false, true);
        if (locSpawnLocation == null)
        {
            locSpawnLocation = locCurrentLocation;
        }
        obj_id objCreatedTemplate = createObject(MUSTAFAR_FALLBACK_LAIR_TEMPLATE, locSpawnLocation);
        ...
        setObjVar(objCreatedTemplate, "spawning.intDifficultyLevel", intLevel);
        setObjVar(objCreatedTemplate, "spawning.lairDifficulty", create.getLairDifficulty(0, 0, intLevel));
        setObjVar(objCreatedTemplate, "spawning.lairType", getMustafarFallbackEncounterLair());
        setObjVar(objCreatedTemplate, "spawning.buildingTrackingType", "");
        ...
        int intSpawnedTemplates = getIntObjVar(self, "spawning.intSpawnedTemplates");
        intSpawnedTemplates++;
        setObjVar(self, "spawning.intSpawnedTemplates", intSpawnedTemplates);
    }
    public String getMustafarFallbackEncounterLair() throws InterruptedException
    {
        String[] strFallbackLairs =
        {
        ...
        return strFallbackLairs[rand(0, strFallbackLairs.length - 1)];
    }
    public int spawn_Trigger(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (locTest != null && locTest.area != null && !locTest.area.equals(MUSTAFAR_PLANET) && utils.checkConfigFlag("ScriptFlags", "newSpawnSystem_" + locTest.area))
        {
