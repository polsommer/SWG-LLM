...
public class newsnet_terminal extends script.base_script {
    ...
    public static final float[] RATINGS = {
        0.10f, 0.25f, 0.50f, 0.75f, 1.0f, 1.25f, 1.5f, 2.0f, 2.5f
    };

    public static final String[] RATING_STRINGS = {
        "rebel_losing_4", "rebel_losing_3", "rebel_losing_2", "rebel_losing_1",
        "equal", "rebel_winning_1", "rebel_winning_2", "rebel_winning_3", "rebel_winning_4"
    };

    public static final String[] BARK_STRINGS = {
        "newsnet_extra", "newsnet_read_all"
    };
    ...
    public String getHeadlineString(float fltRatio) throws InterruptedException {
        if (fltRatio <= RATINGS[0]) return RATING_STRINGS[0];
        else if (fltRatio <= RATINGS[1]) return RATING_STRINGS[1];
        else if (fltRatio <= RATINGS[2]) return RATING_STRINGS[2];
        else if (fltRatio <= RATINGS[3]) return RATING_STRINGS[3];
        else if (fltRatio <= RATINGS[5]) return RATING_STRINGS[4];
        else if (fltRatio <= RATINGS[6]) return RATING_STRINGS[5];
        else if (fltRatio <= RATINGS[7]) return RATING_STRINGS[6];
        else if (fltRatio <= RATINGS[8]) return RATING_STRINGS[7];
        else return RATING_STRINGS[8];
    }
