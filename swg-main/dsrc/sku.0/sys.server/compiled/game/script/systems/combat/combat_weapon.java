    ...
    public int OnGetAttributes(obj_id self, obj_id player, String[] names, String[] attribs) throws InterruptedException
    {
        ...
        if (template != null)
        {
            ...
            if (staticItem)
            {
                ...
                if (itemData == null)
                {
                    LOG("combat_weapon", "Missing master static item dictionary for " + self + " (" + template + ") while building attributes.");
                }
            }
            int levelRequired = -1;
        ...
        if (staticItem)
        {
            ...
            {
                LOG("combat_weapon", "Missing static weapon dictionary for " + self + " (" + getTemplateName(self) + ") while building proc attributes.");
            }
        }
        return SCRIPT_CONTINUE;
    }
