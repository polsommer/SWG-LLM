...
public int handleEnzymeLootToggleConfirmation(obj_id self, dictionary params) throws InterruptedException {
    ...
    if (message.length() > 0) {
        sendSystemMessage(self, new string_id("enzyme_system", message));
    }
    return SCRIPT_CONTINUE;
}


    public int EnzymeLootToggle(obj_id self, obj_id target, String params, float defaultTime) throws InterruptedException
    {
        if (sui.hasPid(self, "enzymeLootToggle"))
        {
