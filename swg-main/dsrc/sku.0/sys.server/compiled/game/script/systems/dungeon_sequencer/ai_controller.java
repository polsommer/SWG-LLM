...
public class ai_controller extends script.base_script
{
    ...
    public void handleDeathTrigger(obj_id self, obj_id killer) throws InterruptedException
    {
        ...
        recordGuildPveKill(self, killer);
    ...
    private void recordGuildPveKill(obj_id self, obj_id killer) throws InterruptedException
    {
        if (!isIdValid(self))
        {
            return;
        }
        String eventId = getStringObjVar(self, "guildPveEventId");
        if (eventId == null || eventId.length() == 0)
        {
            return;
        }
        obj_id contributor = resolveGuildPveContributor(killer);
        if (!isIdValid(contributor))
        {
            return;
        }
        if (hasObjVar(self, "guildPveRequiresGroup") && getIntObjVar(self, "guildPveRequiresGroup") == 1)
        {
            if (!isValidGuildGroup(contributor))
            {
                return;
            }
        }
        int amount = 1;
        if (hasObjVar(self, "guildPveContribution"))
        {
            amount = Math.max(1, getIntObjVar(self, "guildPveContribution"));
        }
        guild.recordGuildPveContribution(contributor, eventId, amount, "dungeon_sequencer");
    }
    private obj_id resolveGuildPveContributor(obj_id killer) throws InterruptedException
    {
        if (!isIdValid(killer))
        {
    ...
    private boolean isValidGuildGroup(obj_id player) throws InterruptedException
    {
