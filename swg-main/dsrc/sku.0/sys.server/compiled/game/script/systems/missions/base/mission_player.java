...
public class mission_player extends script.systems.missions.base.mission_player_base
{
        ...
        if (hasObjVar(objMissionTerminal, "intBounty"))
        {
            ...
            for (obj_id objMissionDatum : objMissionData) {
                ...
                if (objTest == null) {
                    ...
                    retuneMissionForPlayer(objTest, self);
                }
                pvpBountyCount++;
            }
        }
    ...
    private void retuneMissionForPlayer(obj_id missionData, obj_id player) throws InterruptedException
    {
        if (!isIdValid(missionData) || !exists(missionData) || !isIdValid(player) || !exists(player))
        {
            return;
        }

        int baseReward = Math.max(0, getMissionReward(missionData));
        int baseDifficulty = Math.max(0, getMissionDifficulty(missionData));

        int playerLevel = 1;
        try
        {
            playerLevel = Math.max(1, getLevel(player));
        }
        ...
        int groupSize = getEffectiveGroupSize(player);

        float levelFactor = 0.85f + Math.min(1.35f, playerLevel / 60.0f);
        if (levelFactor < 0.6f)
        {
            levelFactor = 0.6f;
        }
        ...
        if (baseDifficulty > 0)
        {
            difficultyFactor += Math.min(0.8f, baseDifficulty / 45.0f);
        }

        float groupRewardBonus = 1.0f;
        float groupDifficultyBonus = 1.0f;
        if (groupSize > 1)
        {
            groupRewardBonus += Math.min(0.75f, (groupSize - 1) * 0.2f);
            groupDifficultyBonus += Math.min(0.6f, (groupSize - 1) * 0.18f);
        }

        float variance = (float)rand(95, 110) / 100.0f;

        int tunedReward = Math.round(baseReward * levelFactor * difficultyFactor * groupRewardBonus * variance);
        if (tunedReward < baseReward)
        {
            tunedReward = baseReward;
        }

        setMissionReward(missionData, tunedReward);

        if (baseDifficulty <= 0)
        {
            baseDifficulty = Math.max(5, playerLevel / 2);
        }

        int tunedDifficulty = Math.round(baseDifficulty * groupDifficultyBonus);
        if (groupSize > 1 && tunedDifficulty <= baseDifficulty)
        {
            tunedDifficulty = baseDifficulty + groupSize;
        }
        setMissionDifficulty(missionData, tunedDifficulty);

        if (!hasObjVar(missionData, "originalGroupSize"))
        {
            setObjVar(missionData, "originalGroupSize", groupSize);
        }
    }

    private int getEffectiveGroupSize(obj_id player) throws InterruptedException
    {
        ...
        int size = getPCGroupSize(groupObject);
        if (size < 1)
        {
            size = 1;
        }
