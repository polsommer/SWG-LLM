...
public class player_utility extends script.base_script
{
    ...
    public int OnRequestAuctionFee(obj_id self, obj_id who, obj_id bazaar, obj_id item, boolean premium, modifiable_int amount) throws InterruptedException
    {
        ...
        boolean trustedAdaptiveNpcListing = isTrustedAdaptiveNpcListing(who, item);
        if (trustedAdaptiveNpcListing)
        {
            amount.set(0);
            behavior_telemetry.recordNpcOutcome(who, "listing_fee_waived", true);
            setObjVar(who, "npc.simProfile.vendor.listing.listing_fee_waived", Math.max(0, getIntObjVar(who, "npc.simProfile.vendor.listing.listing_fee_waived")) + 1);
            return SCRIPT_CONTINUE;
        }
        ...
        int adjustedFee = vendor_lib.applyEconomyAuctionFeeTuning(bazaar, item, self, baseFee);
        prose_package pp = prose.getPackage(SID_SALE_FEE, adjustedFee);
        sendSystemMessageProse(self, pp);
        if (adjustedFee < baseFee)
        {
            String categoryLabel = hasObjVar(self, "vendor.shortage.lastCategoryLabel") ? getStringObjVar(self, "vendor.shortage.lastCategoryLabel") : "Shortage";
            sendSystemMessage(self, "Shortage incentive applied (" + categoryLabel + "). Listing fee reduced from " + baseFee + " to " + adjustedFee + " credits.", "");
        }
        amount.set(adjustedFee);
        logBalance("auctionFee;" + who + ";" + bazaar + ";" + adjustedFee);
        return SCRIPT_CONTINUE;
    }

    private boolean isTrustedAdaptiveNpcListing(obj_id who, obj_id item) throws InterruptedException
    {
        if (!isIdValid(who) || !isIdValid(item) || !exists(who) || !exists(item))
        {
            return false;
        }
        if (!ai_lib.isNpc(who) || !hasScript(who, "npc.simulation.adaptive_archetype_controller"))
        {
            return false;
        }
        String source = hasObjVar(item, "vendor.listingSource") ? getStringObjVar(item, "vendor.listingSource") : null;
        if (source == null || !source.equals(vendor_lib.AI_LISTING_SOURCE))
        {
            return false;
        }
        if (!hasObjVar(item, vendor_lib.AI_LISTING_FLAG) || getIntObjVar(item, vendor_lib.AI_LISTING_FLAG) != 1)
        {
            return false;
        }
        if (hasObjVar(item, "vendor.listingCreator"))
        {
            obj_id creator = getObjIdObjVar(item, "vendor.listingCreator");
            if (!isIdValid(creator) || creator != who)
            {
                return false;
            }
        }
        return hasObjVar(item, "vendor.npcListing") && getIntObjVar(item, "vendor.npcListing") == 1;
    }
