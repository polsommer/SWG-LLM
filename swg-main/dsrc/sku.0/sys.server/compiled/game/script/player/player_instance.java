...
public class player_instance extends script.base_script
{
    ...
    public int handleAwardtoken(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        if (tokenIndex < 0 || tokenIndex >= tokenmanager.HEROIC_TOKENS.length)
        {
            sendSystemMessageTestingOnly(self, "DEBUG: handleAwardtoken sent with out of range tokenIndex.");
            return SCRIPT_CONTINUE;
        }
        obj_id inventory = utils.getInventoryContainer(self);
        obj_id token = static_item.createNewItemFunction(tokenmanager.HEROIC_TOKENS[tokenIndex], inventory, count);
        prose_package pp = new prose_package();
        if (count > 1)
        {
        ...
        pp = prose.setTT(pp, new string_id("static_item_n", tokenmanager.HEROIC_TOKENS[tokenIndex]));
        sendSystemMessageProse(self, pp);
