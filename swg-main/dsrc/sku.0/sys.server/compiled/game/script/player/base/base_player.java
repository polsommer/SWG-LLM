...
public class base_player extends script.base_script
{
    ...
    private static final String PLACEMENT_HELPER_BASE_VAR = "placementHelper";
    private static final String PLACEMENT_HELPER_ITEM_VAR = PLACEMENT_HELPER_BASE_VAR + ".item";
    private static final String PLACEMENT_HELPER_PID_VAR = PLACEMENT_HELPER_BASE_VAR + ".pid";
    private static final String PLACEMENT_HELPER_STEP_INDEX_VAR = PLACEMENT_HELPER_BASE_VAR + ".stepIndex";
    private static final String PLACEMENT_HELPER_CELL_VAR = PLACEMENT_HELPER_BASE_VAR + ".cell";
    private static final String PLACEMENT_HELPER_ACTIVE_VAR = PLACEMENT_HELPER_BASE_VAR + ".active";
    private static final float[] PLACEMENT_HELPER_STEPS =
    {
        0.05f,
        0.25f,
        1.0f
    };
    private static final String[] PLACEMENT_HELPER_STEP_NAMES =
    {
        "Fine",
        "Standard",
        "Wide"
    };
    private static final float PLACEMENT_HELPER_ROTATE_STEP = 5.0f;
    private static final int CMD_MOVE_FURNITURE = getStringCrc("moveFurniture");
    private static final int CMD_ROTATE_FURNITURE = getStringCrc("rotateFurniture");
    ...
    public int requestMissionLocations(obj_id self, dictionary params) throws InterruptedException
    {
        ...
        // Clamp requested number (never negative)
        int requestNum = params.getInt("requestMissionLocationsNumber");
        if (requestNum < 0)
        {
            requestNum = 0;
        }

        // Collect eligible mission start locations
        obj_id[] missions = getMissionObjects(self);
        Vector missionLocations = new Vector(); // utils.addElement works with Vector

        if (missions != null && requestNum != 0)
        {
            int collected = 0;
            for (obj_id mission : missions)
            {
                if (!isIdValid(mission)) { continue; }
                if (getMissionStatus(mission) <= 0) { continue; }

                utils.addElement(missionLocations, getMissionStartLocation(mission));
                collected++;

                if (requestNum > 0 && collected >= requestNum)
                {
                    break; // stop once we've satisfied the request
                }
            }
        }
        ...
        response.put("requestMissionLocationsNumber", requestNum);
        response.put("sender", self);
        response.put("senderLocation", getLocation(self));
        if (missionLocations.size() > 0)
        {
            response.put("missionLocation", missionLocations);
        }

        // Send to group and clean up waypoints
        messageTo(groupObject, "missionLocationResponse", response, 0.0f, false);
        group.destroyGroupWaypoint(self);

        return SCRIPT_CONTINUE;
    }
    /**
     * Ensures group-member script is attached and notifies the player's beast (if any)
     * that the owner has joined a group.
     */
    public int OnAddedToGroup(obj_id self, obj_id groupId) throws InterruptedException
    {
        // Attach the group member script if missing
        if (!hasScript(self, group.SCRIPT_GROUP_MEMBER))
        {
            attachScript(self, group.SCRIPT_GROUP_MEMBER);
        }

        // Notify beast controller data (BCD) if the player has a valid beast out
        final float MSG_DELAY_SEC = 1.0f;

        obj_id beast = beast_lib.getBeastOnPlayer(self);
        if (isIdValid(beast) && exists(beast))
        {
            obj_id beastBCD = beast_lib.getBeastBCD(beast);
            if (isIdValid(beastBCD) && exists(beastBCD))
            {
                messageTo(beastBCD, "ownerGrouped", null, MSG_DELAY_SEC, false);
            }
        }
    ...
    /**
     * Fired when the player leaves a group (voluntarily or kicked).
     * Cleans up holograms, rally points, and group-member script.
     */
    public int OnRemovedFromGroup(obj_id self, obj_id groupId) throws InterruptedException
    {
        // If a hologram performance was active, clean it up and clear the marker var.
        if (utils.hasScriptVar(self, "currentHolo"))
        {
            performance.holographicCleanup(self);
            utils.removeScriptVar(self, "currentHolo");
        }

        // Clear any active rally point effects from Squad Leader.
        squad_leader.clearRallyPoint(self);

        // Detach group-member script only if it's actually attached (avoids noisy logs).
        if (hasScript(self, group.SCRIPT_GROUP_MEMBER))
        {
            detachScript(self, group.SCRIPT_GROUP_MEMBER);
        }
    ...
    /**
     * Fired when the group leader changes. No-op by design.
     * Consider adding UI refresh or permission cache invalidation here if needed.
     */
    ...
    /**
     * Fired when the entire group disbands.
     * Mirrors OnRemovedFromGroup’s rally cleanup but does not touch scripts (caller may still be in other flows).
     */
    ...
    /**
     * Builds the object menu for interactions with a player.
     * Adds deathblow when target is incapacitated and attackable.
     * Adds revive/resuscitate options when target is dead and in group or has consent.
     * Adds performance listen/watch controls based on current performance state.
     */
    public int OnObjectMenuRequest(final obj_id self, final obj_id player, final menu_info mi) throws InterruptedException
    {
        // Defensive: bail if menu container is missing
        if (mi == null)
        {
            return SCRIPT_CONTINUE;
        }

        // Clear "noTrade" marker if present (avoid noisy writes if absent)
        if (hasObjVar(self, "noTrade"))
        {
            removeObjVar(self, "noTrade");
        }

        // --- Combat / incapacitation options ---
        // If a COMBAT_DEATH_BLOW entry already exists, just mark for server notify.
        menu_info_data deathBlowItem = mi.getMenuItemByType(menu_info_types.COMBAT_DEATH_BLOW);
        if (deathBlowItem != null)
        {
            deathBlowItem.setServerNotify(true);
        }
        else
        {
            int posture = getPosture(self);

            // Incapacitated: offer deathblow if the player can attack the target.
            if (posture == POSTURE_INCAPACITATED)
            {
            ...
            // Dead: offer revive/resuscitate to groupmates or with consent.
            else if (posture == POSTURE_DEAD)
            {
                boolean canAssist = group.inSameGroup(self, player) || pclib.hasConsent(player, self);
                if (canAssist)
                {
                    boolean hasRevive      = hasCommand(player, "revivePlayer");
                    boolean hasResuscitate = hasCommand(player, "resuscitatePlayer");
        ...
        // --- Performance listen/watch controls ---
        // Listen controls
        ...
        if (hasScript(self, performance.MUSIC_HEARTBEAT_SCRIPT) &&
                (!isIdValid(listenTarget) || !group.inSameGroup(self, listenTarget)))
        {
    ...
    public int OnLogin(obj_id self) throws InterruptedException
    {
        ...
        if (!hasScript(self, "player.species_perk_manager"))
        {
            attachScript(self, "player.species_perk_manager");
        }
        else
        {
            messageTo(self, "refreshSpeciesPerks", null, 1.0f, false);
        }
        if (!hasScript(self, "player.player_behavior_telemetry"))
        {
            attachScript(self, "player.player_behavior_telemetry");
        }
        obj_id behaviorPlanet = getPlanetByName(getLocation(self).area);
        if (isIdValid(behaviorPlanet) && !hasScript(behaviorPlanet, "systems.behavior.profile_builder"))
        {
            attachScript(behaviorPlanet, "systems.behavior.profile_builder");
        }
        if (!hasScript(self, "systems.jedi.enclave_master"))
        {
            attachScript(self, "systems.jedi.enclave_master");
        }
        else
        {
            messageTo(self, "handlePlayerLogin", null, 1.0f, false);
        }

        if (!hasScript(self, "live_event.map_dressing_zone_entry"))
        {
            attachScript(self, "live_event.map_dressing_zone_entry");
        }
    ...
    public int OnLostItem(obj_id self, obj_id destContainer, obj_id transferer, obj_id item) throws InterruptedException
    {
        ...
        if (shouldOfferPlacementHelper(self, destContainer, transferer, item))
        {
            requestPlacementHelper(self, item, destContainer);
        }
        return SCRIPT_CONTINUE;
    }
    ...
    private boolean shouldOfferPlacementHelper(obj_id player, obj_id destContainer, obj_id transferer, obj_id item) throws InterruptedException
    {
        if (!isIdValid(player) || !isIdValid(item) || !exists(item))
        {
            return false;
        }
        if (!isIdValid(destContainer) || transferer != player)
        {
            return false;
        }
        String cellTemplate = getTemplateName(destContainer);
        if (cellTemplate == null || !cellTemplate.equals(structure.TEMPLATE_CELL))
        {
            return false;
        }
        obj_id structure = getTopMostContainer(destContainer);
        if (!isIdValid(structure) || !player_structure.isBuilding(structure))
        {
            return false;
        }
        location playerLoc = getLocation(player);
        if (playerLoc == null || !isIdValid(playerLoc.cell) || playerLoc.cell != destContainer)
        {
            return false;
        }
        ...
        if (!canManipulateSkipNoTradeCheck(player, item, true, false, 0, true))
        {
            return false;
        }
        if (hasCondition(item, CONDITION_VENDOR) || hasObjVar(item, "noMoveItem"))
        {
            return false;
        }
        obj_id structureObject = getTopMostContainer(item);
        if (structure != structureObject || structureObject == item)
        {
            return false;
        }
        obj_id[] baseObjects = player_structure.getStructureBaseObjects(structure);
        if (baseObjects != null)
        {
            for (obj_id baseObject : baseObjects)
            {
                if (baseObject == item)
                {
                    return false;
        ...
        int got = getGameObjectType(item);
        if (!isGameObjectTypeOf(got, GOT_misc_furniture)
                && !isGameObjectTypeOf(got, GOT_misc_item)
                && !isGameObjectTypeOf(got, GOT_misc_item_usable))
        {
            return false;
        }
        return true;
    }
    private void requestPlacementHelper(obj_id player, obj_id item, obj_id cell) throws InterruptedException
    {
        int stepIndex = 1;
        if (utils.hasScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR))
        {
            stepIndex = utils.getIntScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR);
        }
        cleanupPlacementHelper(player);
        utils.setScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR, stepIndex);
        utils.setScriptVar(player, PLACEMENT_HELPER_ITEM_VAR, item);
        utils.setScriptVar(player, PLACEMENT_HELPER_CELL_VAR, cell);
        dictionary params = new dictionary();
        params.put("item", item);
        messageTo(player, "startPlacementHelper", params, 0.1f, false);
    }
    public int startPlacementHelper(obj_id self, dictionary params) throws InterruptedException
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
        ...
        obj_id expectedCell = utils.getObjIdScriptVar(self, PLACEMENT_HELPER_CELL_VAR);
        location itemLoc = getLocation(item);
        if (expectedCell != null && expectedCell != obj_id.NULL_ID)
        {
            if (itemLoc == null || !isIdValid(itemLoc.cell) || itemLoc.cell != expectedCell)
            {
                cleanupPlacementHelper(self);
                return SCRIPT_CONTINUE;
            }
        }
        if (utils.hasScriptVar(self, PLACEMENT_HELPER_PID_VAR))
        {
            int existingPid = utils.getIntScriptVar(self, PLACEMENT_HELPER_PID_VAR);
            sui.closeSUI(self, existingPid);
            utils.removeScriptVar(self, PLACEMENT_HELPER_PID_VAR);
        }
        utils.setScriptVar(self, PLACEMENT_HELPER_ITEM_VAR, item);
        setLookAtTarget(self, item);
        faceTo(self, item);
        String[] options = getPlacementHelperOptions(self);
        if (options == null || options.length == 0)
        {
            cleanupPlacementHelper(self);
            return SCRIPT_CONTINUE;
        }
        String prompt = "Fine tune the placement of the item with the options below.";
        int pid = sui.listbox(self, self, prompt, sui.OK_CANCEL, "Placement Helper", options, "handlePlacementHelper", true, true);
        if (pid > -1)
        {
            setSUIAssociatedObject(pid, item);
            setSUIMaxRangeToObject(pid, 32.0f);
            utils.setScriptVar(self, PLACEMENT_HELPER_PID_VAR, pid);
            if (!utils.hasScriptVar(self, PLACEMENT_HELPER_ACTIVE_VAR))
            {
                utils.setScriptVar(self, PLACEMENT_HELPER_ACTIVE_VAR, true);
                sendSystemMessage(self, "Placement helper active. Adjust the item or cancel when finished.", null);
            }
        }
        else
        {
    ...
    public int handlePlacementHelper(obj_id self, dictionary params) throws InterruptedException
    {
        if (params == null || params.isEmpty())
        {
            cleanupPlacementHelper(self);
        ...
        obj_id item = utils.getObjIdScriptVar(self, PLACEMENT_HELPER_ITEM_VAR);
        if (!isIdValid(item) || !exists(item))
        {
            cleanupPlacementHelper(self);
            return SCRIPT_CONTINUE;
        }
        boolean keepActive = performPlacementHelperAction(self, item, row);
        if (keepActive)
        {
            reopenPlacementHelper(self, item);
        }
        else
        {
            cleanupPlacementHelper(self);
    ...
    private void reopenPlacementHelper(obj_id player, obj_id item) throws InterruptedException
    {
        dictionary params = new dictionary();
        params.put("item", item);
        messageTo(player, "startPlacementHelper", params, 0.1f, false);
    }
    private boolean performPlacementHelperAction(obj_id player, obj_id item, int selection) throws InterruptedException
    {
        float step = getPlacementHelperStep(player);
        int centimeters = Math.max(1, Math.round(step * 100.0f));
        switch (selection)
        {
            case 0:
            {
                sendSystemMessage(player, "Placement helper closed.", null);
                return false;
            }
            case 1:
            {
                queuePlacementMove(player, item, "FORWARD", centimeters);
                return true;
            }
            case 2:
            {
                queuePlacementMove(player, item, "BACK", centimeters);
                return true;
            }
            case 3:
            {
                queuePlacementMove(player, item, "LEFT", centimeters);
                return true;
            }
            case 4:
            {
                queuePlacementMove(player, item, "RIGHT", centimeters);
                return true;
            }
            case 5:
            {
                queuePlacementMove(player, item, "UP", centimeters);
                return true;
            }
            case 6:
            {
                queuePlacementMove(player, item, "DOWN", centimeters);
                return true;
            }
            case 7:
            {
                queuePlacementRotate(player, item, -PLACEMENT_HELPER_ROTATE_STEP);
                return true;
            }
            case 8:
            {
            ...
            case 9:
            {
                cyclePlacementHelperStep(player);
                float newStep = getPlacementHelperStep(player);
                sendSystemMessage(player, "Move increment set to " + formatMeters(newStep) + "m.", null);
                return true;
            }
            default:
            {
    ...
    private void queuePlacementMove(obj_id player, obj_id item, String direction, int centimeters) throws InterruptedException
    {
        String params = direction + " " + centimeters + " " + item;
        if (!queueCommand(player, CMD_MOVE_FURNITURE, item, params, COMMAND_PRIORITY_DEFAULT))
        {
            sendSystemMessage(player, "Unable to adjust the item. Try again in a moment.", null);
        }
    }
    private void queuePlacementRotate(obj_id player, obj_id item, float degrees) throws InterruptedException
    {
        int rotation = Math.round(degrees);
        if (rotation == 0)
        {
            rotation = degrees > 0 ? 1 : -1;
        }
        String params = "yaw " + rotation + " " + item;
        if (!queueCommand(player, CMD_ROTATE_FURNITURE, item, params, COMMAND_PRIORITY_DEFAULT))
        {
            sendSystemMessage(player, "Unable to rotate the item right now.", null);
        }
    }
    private float getPlacementHelperStep(obj_id player) throws InterruptedException
    {
        int index = 1;
        if (utils.hasScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR))
        {
            index = utils.getIntScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR);
        }
        if (index < 0 || index >= PLACEMENT_HELPER_STEPS.length)
        {
            index = 1;
            utils.setScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR, index);
        }
        return PLACEMENT_HELPER_STEPS[index];
    }
    private String getPlacementHelperStepLabel(obj_id player) throws InterruptedException
    {
        int index = 1;
        if (utils.hasScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR))
        {
        ...
        if (index < 0 || index >= PLACEMENT_HELPER_STEP_NAMES.length)
        {
            index = 1;
        }
        return PLACEMENT_HELPER_STEP_NAMES[index];
    }
    private void cyclePlacementHelperStep(obj_id player) throws InterruptedException
    {
        int index = 1;
        if (utils.hasScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR))
        {
        ...
        index++;
        if (index >= PLACEMENT_HELPER_STEPS.length)
        {
            index = 0;
        }
        utils.setScriptVar(player, PLACEMENT_HELPER_STEP_INDEX_VAR, index);
    }
    private String[] getPlacementHelperOptions(obj_id player) throws InterruptedException
    {
        float step = getPlacementHelperStep(player);
        String stepLabel = formatMeters(step);
        String rotateLabel = String.format(Locale.US, "%.1f", PLACEMENT_HELPER_ROTATE_STEP);
        String currentStepName = getPlacementHelperStepLabel(player);
        return new String[]
        {
            "Finish placement",
            "Move forward (+" + stepLabel + "m)",
            "Move backward (-" + stepLabel + "m)",
            "Move left (-" + stepLabel + "m)",
            "Move right (+" + stepLabel + "m)",
            "Raise (+" + stepLabel + "m)",
            "Lower (-" + stepLabel + "m)",
            "Rotate left (-" + rotateLabel + "°)",
            "Rotate right (+" + rotateLabel + "°)",
            "Toggle move increment (current: " + currentStepName + ")"
        };
    }
    ...
    private void cleanupPlacementHelper(obj_id player) throws InterruptedException
    {
        if (!isIdValid(player) || !exists(player))
        {
            return;
        }
        if (utils.hasScriptVar(player, PLACEMENT_HELPER_PID_VAR))
        {
            int pid = utils.getIntScriptVar(player, PLACEMENT_HELPER_PID_VAR);
            sui.closeSUI(player, pid);
        }
        utils.removeScriptVarTree(player, PLACEMENT_HELPER_BASE_VAR);
        setLookAtTarget(player, obj_id.NULL_ID);
    }
                        ...
                        if (componentsDictionary != null)
                        {
                            ...
                            for (Integer slot : slots) {
                                ...
                                if (isIdValid(newComponent)) {
                                    if (shipCanInstallComponent(newItem, slot, newComponent)) {
                                        if (!shipInstallComponent(player, newItem, slot, newComponent)) {
                                            LOG("ShipComponents", "Failed to install component " + newComponent + " into slot " + slot + " on ship " + newItem + " during unpack.");
                                        }
                                    } else {
                                        LOG("ShipComponents", "Skipping install of component " + newComponent + " into slot " + slot + " on ship " + newItem + " during unpack: slot not available.");
                                    }
                                }
                            }
                        }
                    ...
                    for (int shipSlot : shipSlots) {
                        if (isShipSlotInstalled(item, shipSlot)) {
                            ...
                            if (isIdValid(component)) {
                                ...
                                if (shipCanInstallComponent(item, shipSlot, component)) {
                                    if (!shipInstallComponent(getOwner(item), item, shipSlot, component)) {
                                        LOG("ShipComponents", "Failed to reinstall component " + component + " into slot " + shipSlot + " on ship " + item + " after packing.");
                                    }
                                } else {
                                    LOG("ShipComponents", "Skipping reinstall of component " + component + " into slot " + shipSlot + " on ship " + item + " after packing: slot not available.");
                                }
                            }
                        }
                    }
    ...
    public void createStartingEquipment(obj_id objPlayer) throws InterruptedException
    {
        ...
        if (gender == GENDER_MALE)
        {
            if (species == SPECIES_HUMAN
                    || species == SPECIES_ZABRAK
                    || species == SPECIES_BOTHAN
                    || species == SPECIES_MON_CALAMARI
                    || species == SPECIES_RODIAN
                    || species == SPECIES_TWILEK
                    || species == SPECIES_AQUALISH
                    || species == SPECIES_ARCONA
                    || species == SPECIES_BITH
                    || species == SPECIES_DEVARONIAN
                    || species == SPECIES_MARAUDER
                    || species == SPECIES_NIGHTSISTER
                    || species == SPECIES_NAUTOLAN
                    || species == SPECIES_TOGRUTA
                    || species == SPECIES_WEEQUAY
                    || species == SPECIES_ISHI_TIB)
            {
                idx = 0;
            }
