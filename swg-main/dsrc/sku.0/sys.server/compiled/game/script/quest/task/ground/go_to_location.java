...
public class go_to_location extends script.quest.task.ground.base_task
{
    ...
    public int OnTaskActivated(obj_id self, int questCrc, int taskId) throws InterruptedException
    {
        ...
        String baseObjVar = groundquests.setBaseObjVar(self, taskType, questName, taskId);
        String objvarTaskIdName = baseObjVar + dot + objvarTaskId;
    ...
    public int OnArrivedAtLocation(obj_id self, String locationName) throws InterruptedException
    {
        ...
        String baseObjVar = taskId >= 0 ? groundquests.getBaseObjVar(self, taskType, questName, taskId) : null;
        String legacyObjvarName = objvarBase + dot + questName;
        boolean result = (baseObjVar != null && hasObjVar(self, baseObjVar)) || hasObjVar(self, legacyObjvarName);
        if (result)
        {
            if (taskId == -1)
            {
                String objvarTaskIdName = legacyObjvarName + dot + objvarTaskId;
                if (baseObjVar != null && hasObjVar(self, baseObjVar + dot + objvarTaskId))
                {
                    objvarTaskIdName = baseObjVar + dot + objvarTaskId;
                }
