...
public class wait_for_tasks extends script.quest.task.ground.base_task
{
    ...
    public int OnTaskActivated(obj_id self, int questCrc, int taskId) throws InterruptedException
    {
        ...
        while (!done)
        {
            ...
            if (questName != null && questName.length() > 0)
            {
                ...
                if (taskName != null && taskName.length() > 0)
                {
                    ...
                    if (taskQuestCrc > 0 && taskTaskId >= 0)
                    {
                    ...
                    {
                        groundquests.questOutputDebugInfo(self, questCrc, taskId, taskType, "OnTaskActivated", "[designer bug] Invalid quest/task pair for wait_for_tasks: " + questName + ":" + taskName);
                        ++waitForTaskIndex;
                    }
                }
                else 
        ...
        if (questCrcsListResizable.size() == taskIdsListResizable.size())
        {
            ...
            {
                String activeQuestName = questGetQuestName(questCrc);
                String activeTaskName = groundquests.getTaskStringDataEntry(questCrc, taskId, groundquests.dataTableColumnTaskName);
                ...
                groundquests.questOutputDebugInfo(self, questCrc, taskId, taskType, "OnTaskActivated", "Auto-failing wait_for_tasks to prevent stuck progression. Quest=" + activeQuestName + " task=" + activeTaskName + " (taskId=" + taskId + ")");
        ...
        while (!done)
        {
            ...
            if (questName != null && questName.length() > 0)
            {
                ...
                if (taskName != null && taskName.length() > 0)
                {
                    ...
                    int taskQuestCrc = groundquests.getQuestIdFromString(questName);
                    int taskTaskId = groundquests.getTaskId(taskQuestCrc, taskName);
                    if (taskQuestCrc > 0 && taskTaskId >= 0 && questIsTaskComplete(taskQuestCrc, taskTaskId, self))
                    {
                        handleSomeTaskFinished(self, taskQuestCrc, taskTaskId);
                    }
                    else if (taskQuestCrc <= 0 || taskTaskId < 0)
                    {
                        groundquests.questOutputDebugInfo(self, questCrc, taskId, taskType, "checkForAlreadyCompletedTasks", "[designer bug] Invalid quest/task pair for wait_for_tasks: " + questName + ":" + taskName);
                    }
                    ++waitForTaskIndex;
                }
                else 
