...
public class fishing extends script.base_script {
    ...
    public static final double ELUSIVE_FISH_CHANCE = 0.005d; // 0.5% chance default
    ...
    public static String getFishSpecies(obj_id fish) throws InterruptedException {
        ...
        if (name.startsWith("col_fish_") && parts.length >= 4) {
            return parts[3];
        }
    ...
    public static String getFishPlanet(obj_id fish) throws InterruptedException {
        ...
        String[] parts = name.split("_");
        if (name.startsWith("col_fish_") && parts.length >= 3) {
            if (parts.length >= 4 && "rare".equals(parts[2])) {
                return "";
            }
            return parts[2];
        }
