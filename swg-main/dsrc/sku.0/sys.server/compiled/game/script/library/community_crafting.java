...
public class community_crafting extends script.base_script
{
    ...
    public static boolean handleNpcCraftingAssist(obj_id craftingTracker, dictionary params) throws InterruptedException
    {
        obj_id crafter = params != null ? params.getObjId("crafter") : null;
        if (!isIdValid(craftingTracker) || !isIdValid(crafter))
        {
            return false;
        }
        if (!ai_lib.isNpc(crafter))
        {
            writeNpcCraftingOutcome(crafter, craftingTracker, false, "sender_not_npc");
            return false;
        }
        if (!isInitializedForCC(craftingTracker) || !isSessionActive(craftingTracker))
        {
            writeNpcCraftingOutcome(crafter, craftingTracker, false, "session_inactive");
            return false;
        }
        if (!isPlayerCrafting(craftingTracker, crafter) && !addPlayerToSystem(craftingTracker, crafter))
        {
            writeNpcCraftingOutcome(crafter, craftingTracker, false, "join_failed");
            return false;
        }

        int quantity = Math.max(1, Math.min(3, rand(1, 3)));
        float quality = ((float)rand(30, 90)) / 100.0f;
        String quantityKey = OBJVAR_COMMUNITY_CRAFTING_PLAYER_QUANTITY_TOTAL + "." + crafter;
        String qualityKey = OBJVAR_COMMUNITY_CRAFTING_PLAYER_QUALITY_TOTAL + "." + crafter;
        setObjVar(craftingTracker, quantityKey, Math.max(0, getIntObjVar(craftingTracker, quantityKey)) + quantity);
        setObjVar(craftingTracker, qualityKey, Math.max(0.0f, getFloatObjVar(craftingTracker, qualityKey)) + quality);

        boolean success = quality >= 0.35f;
        writeNpcCraftingOutcome(crafter, craftingTracker, success, success ? "assist_completed" : "assist_failed");
        return success;
    }

    private static void writeNpcCraftingOutcome(obj_id crafter, obj_id tracker, boolean success, String detail) throws InterruptedException
    {
        int now = getGameTime();
        setObjVar(crafter, "npc.simProfile.lastSystem", "crafting");
        setObjVar(crafter, "npc.simProfile.lastAction", "handleNpcCraftingAssist");
        setObjVar(crafter, "npc.simProfile.lastSuccess", success ? 1 : 0);
        setObjVar(crafter, "npc.simProfile.lastDetail", detail);
        setObjVar(crafter, "npc.simProfile.lastTimestamp", now);

        setObjVar(crafter, "npc.simProfile.crafting.lastTracker", tracker);
        setObjVar(crafter, "npc.simProfile.crafting.lastSuccess", success ? 1 : 0);
        setObjVar(crafter, "npc.simProfile.crafting.lastDetail", detail);
        setObjVar(crafter, "npc.simProfile.crafting.lastTimestamp", now);

        utils.setScriptVar(crafter, "npc.simProfile.lastSystem", "crafting");
        utils.setScriptVar(crafter, "npc.simProfile.lastAction", "handleNpcCraftingAssist");
        utils.setScriptVar(crafter, "npc.simProfile.lastSuccess", success ? 1 : 0);
        utils.setScriptVar(crafter, "npc.simProfile.lastDetail", detail);
        utils.setScriptVar(crafter, "npc.simProfile.lastTimestamp", now);
        utils.setScriptVar(crafter, "npc.simProfile.crafting.lastSuccess", success ? 1 : 0);
        utils.setScriptVar(crafter, "npc.simProfile.crafting.lastDetail", detail);
        utils.setScriptVar(crafter, "npc.simProfile.crafting.lastTimestamp", now);
    }
