...
public class loot extends script.base_script
{
    ...
    public static final String HOLOCRON_COLLECTION_COLUMN = "col_jedi_holocron";
    public static final float HOLOCRON_DROP_CHANCE_MULTIPLIER = 1.5f;
    public static final int HOLOCRON_DROP_CHANCE_BONUS = 5;
    public static final int HOLOCRON_SELECTION_WEIGHT_BONUS = 2;
    public static final int HOLOCRON_EXTRA_ROLL_CHANCE = 35;
    public static final int HOLOCRON_MAX_DROPS = 3;
    ...
    {
        if (hasToggledEnzymeLootOff(creature))
    {
        return false;
    } 
    ...
    {
        if (!shouldAttemptChroniclesLoot(target))
        {
            return false;
        }
    ...
    private static boolean shouldAttemptChroniclesLoot(obj_id target) throws InterruptedException
    {
        if (!isIdValid(target))
        {
            return false;
        ...
        if (hasObjVar(target, xp.VAR_TOP_GROUP))
        {
            ...
            if (group.isGroupObject(killCredit))
            {
                ...
                if (groupMembers != null && groupMembers.length > 0)
                {
                    ...
                    {
                        if (isPlayer(groupMember) && !hasToggledChroniclesLootOff(groupMember))
                        {
            ...
            else if (isIdValid(killCredit) && isPlayer(killCredit) && !hasToggledChroniclesLootOff(killCredit))
            {
                hasEligiblePlayer = true;
            }
        }
        ...
        {
            obj_id[] killers = getObjIdArrayObjVar(target, scenario.VAR_PRIMARY_KILLERS);
            if (killers != null && killers.length > 0)
            {
                for (obj_id killer : killers)
                {
                    if (isPlayer(killer) && !hasToggledChroniclesLootOff(killer))
                    {
                        hasEligiblePlayer = true;
                        break;
                    }
    ...
    public static boolean addCollectionLoot(obj_id target, boolean theftBool, obj_id thief) throws InterruptedException
    {
        ...
        String collectionLootData = creatureRow.getString("collectionLoot");
        if (collectionLootData != null && collectionLootData.indexOf(HOLOCRON_COLLECTION_COLUMN) > -1)
        {
            rollBaseChance = applyHolocronChanceModifier(rollBaseChance);
        }
        rollBaseChance *= COL_LOOT_MULTIPLIER_ON;
        if (rollBaseChance > 100)
        {
            rollBaseChance = 100;
        }
        if (rollBaseChance < 0)
        ...
        {
            String myCollectionLoot = collectionLootData;
            ...
            else
            {
                ...
                if (collectionsInColumn == null || collectionsInColumn.length == 0)
                {
                    return false;
                }
                int collectionIndex = getHolocronAwareCollectionIndex(collectionsInColumn);
                myCollectionLoot = collectionsInColumn[collectionIndex];
                if (myCollectionLoot != null)
                {
                    myCollectionLoot = myCollectionLoot.trim();
                }
                if (myCollectionLoot == null || myCollectionLoot.length() == 0)
                {
                    return false;
                }
                ...
                if (isHolocronCollection(myCollectionLoot))
                {
                    return grantHolocronLootDrops(target, theftBool, thief, creatureName, lootArray);
                }
    ...
    public static int getHolocronAwareCollectionIndex(String[] collectionsInColumn) throws InterruptedException
    {
        if (collectionsInColumn == null || collectionsInColumn.length == 0)
        {
            return 0;
        }
        Vector weightedIndexes = new Vector();
        for (int i = 0; i < collectionsInColumn.length; i++)
        {
            String column = collectionsInColumn[i];
            if (column != null)
            {
                column = column.trim();
            }
            ...
            weightedIndexes.add(Integer.valueOf(i));
            if (isHolocronCollection(column))
            {
                for (int weight = 0; weight < HOLOCRON_SELECTION_WEIGHT_BONUS; weight++)
                {
                    weightedIndexes.add(Integer.valueOf(i));
                }
            }
        }
        if (weightedIndexes == null || weightedIndexes.size() == 0)
        {
            return rand(0, collectionsInColumn.length - 1);
        }
        int selection = rand(0, weightedIndexes.size() - 1);
        Integer chosen = (Integer)weightedIndexes.get(selection);
        if (chosen == null)
        {
            return rand(0, collectionsInColumn.length - 1);
        }
    ...
    public static boolean isHolocronCollection(String column)
    {
        if (column == null)
        {
            return false;
        }
        return column.trim().equals(HOLOCRON_COLLECTION_COLUMN);
    }
    public static int applyHolocronChanceModifier(int rollBaseChance)
    {
        float modified = rollBaseChance * HOLOCRON_DROP_CHANCE_MULTIPLIER;
        modified += HOLOCRON_DROP_CHANCE_BONUS;
        if (modified > 100.0f)
        {
            modified = 100.0f;
        }
    ...
    public static boolean grantHolocronLootDrops(obj_id target, boolean theftBool, obj_id thief, String creatureName, String[] lootArray) throws InterruptedException
    {
        if (lootArray == null || lootArray.length == 0)
        {
            return false;
        }
        Vector holocronLoot = new Vector();
        for (int i = 0; i < lootArray.length; i++)
        {
            String entry = lootArray[i];
            if (entry != null)
            {
                entry = entry.trim();
            }
            ...
            holocronLoot.add(entry);
        }
        if (holocronLoot == null || holocronLoot.size() == 0)
        {
            return false;
        }
        int availableDrops = holocronLoot.size();
        int maxDrops = availableDrops;
        if (HOLOCRON_MAX_DROPS > 0 && HOLOCRON_MAX_DROPS < maxDrops)
        {
            maxDrops = HOLOCRON_MAX_DROPS;
        }
        int dropCount = 1;
        while (dropCount < maxDrops && rand(1, 100) <= HOLOCRON_EXTRA_ROLL_CHANCE)
        {
            dropCount++;
        }
        obj_id container;
        if (theftBool)
        {
            ...
            container = utils.getInventoryContainer(thief);
            if (!isIdValid(container))
            {
                return false;
            }
        ...
        {
            container = utils.getInventoryContainer(target);
        }
        if (!isIdValid(container))
        {
            return false;
        ...
        for (int drop = 0; drop < dropCount && holocronLoot.size() > 0; drop++)
        {
            int index = rand(0, holocronLoot.size() - 1);
            String lootToGrant = (String)holocronLoot.remove(index);
            obj_id lootItem = static_item.createNewItemFunction(lootToGrant, container);
            if (isIdValid(lootItem) && exists(lootItem))
            {
                dropped = true;
                if (theftBool)
                {
                    notifyThiefOfItemStolen(container, lootItem);
                }
                CustomerServiceLog("CollectionLootChannel: ", "LootSuccessful: " + creatureName + "(" + target + ")" + " successfully dropped: " + lootToGrant + "(" + lootItem + ")");
            }
            else
...
public static boolean addRareLoot(obj_id target) throws InterruptedException {
    ...
    // Validate the player.
    if (!isValidId(player) || !isPlayer(player) || isAwayFromKeyBoard(player)) {
        return false;
    }

    // Check if RLS is enabled.
    if (!Boolean.parseBoolean(getConfigSetting("GameServer", "rlsEnabled"))) {
        return false;
    }

    // Check basic constraints for time and distance
    if (!meetsBasicConstraints(player, target)) {
        return false;
    }

    // Perform a loot roll to determine the type of chest awarded
    int lootType = getRandomLootType();

    // Get the string representation of the loot type
    String type = getLootTypeName(lootType);

    LOG("random_rare_loot", "Player (" + player + ") qualified for a " + type + " RLS chest!");

    // Award the loot chest and set the relevant player variables.
    awardLootChest(player, target, lootType);

    return true;
}

// Determines loot type based on a simple random roll
private static int getRandomLootType() {
    double roll = Math.random(); // Random roll between 0.0 and 1.0

    // Define thresholds for each loot type
    if (roll <= 0.05) { // 5% chance for Legendary
        return 3;
    } else if (roll <= 0.25) { // 20% chance for Exceptional
        return 2;
...
// Applies basic constraints based on simple time and distance checks
private static boolean meetsBasicConstraints(obj_id player, obj_id target) throws InterruptedException {
    int minDistanceFromLast = getMinDistanceFromLastLoot();
    ...
    if (minDistanceFromLast > 0 && hasObjVar(player, "loot.rls.lastLootedLocation")) {
        ...
        if (distanceFromLast <= minDistanceFromLast) {
            LOG("random_rare_loot", "Player (" + player + ") last looted a chest only " + distanceFromLast + " meters away - too close to this location.");
            return false;
        }
    }

    int rlsMinTimeBetweenAwards = getMinTimeBetweenAwards();
    if (hasObjVar(player, "loot.rls.lastChestAwardTime")) {
        int elapsedTime = getGameTime() - getIntObjVar(player, "loot.rls.lastChestAwardTime");
        if (elapsedTime < rlsMinTimeBetweenAwards) {
            LOG("random_rare_loot", "Player (" + player + ") last looted a chest " + elapsedTime + " seconds ago, which is less than required.");
            return false;
        }
    } else {
        LOG("random_rare_loot", "Player (" + player + ") is looting their first RLS chest!");
    }
    return true;
}

// Retrieves minimum distance from last loot, with a default value if not set
private static int getMinDistanceFromLastLoot() {
    String dist = getConfigSetting("GameServer", "rlsMinDistanceFromLastLoot");
    return dist == null ? 0 : Math.abs(Integer.parseInt(dist));
}

// Retrieves minimum time between loot awards, with a default value if not set
private static int getMinTimeBetweenAwards() {
    String tmpTime = getConfigSetting("GameServer", "rlsMinTimeBetweenAwards");
    return tmpTime == null ? 15 * 60 : Integer.parseInt(tmpTime);
}

// Returns the loot type as a string for logging or display purposes
private static String getLootTypeName(int lootType) {
    switch (lootType) {
        case 3:
            return "LEGENDARY";
        case 2:
...
// Awards the loot chest and sets player variables
private static void awardLootChest(obj_id player, obj_id target, int lootType) throws InterruptedException {
    ...
    setObjVar(player, "loot.rls.lastLootedLocation", getLocation(target));
    playClientEffectLoc(target, RLS_EFFECT, getLocation(target), 1.0f);
    playClientEffectLoc(target, RLS_SOUND, getLocation(target), 1.0f);
}
