...
public class xp extends script.base_script
{
    ...
    public static float getSpeciesXpModifier(obj_id target, String xp_type) throws InterruptedException
    {
        ...
        {
            String[] s;
            Float defaultModifier = null;
            for (String xpMod : xpMods) {
                if (xpMod.startsWith("all:")) {
                    s = split(xpMod, ':');
                    if ((s != null) && (s.length == 2)) {
                        int val = utils.stringToInt(s[1]);
                        if (val != -1) {
                            defaultModifier = (100.0f + val) / 100.0f;
                        }
                    }
                    continue;
                }
    ...
    public static int grantXpByTemplate(obj_id player, int amount) throws InterruptedException
    {
        ...
        {
            amount = grant(player, xp.COMBAT_GENERAL, amount, false);
    ...
    {
        int playerLevel = getLevel(player);
        int maxLevelRows = dataTableGetNumRows(xp.TBL_PLAYER_LEVEL_XP);
        int levelCap = Math.min(130, maxLevelRows - 1);
        if (playerLevel > 0 && playerLevel < levelCap)
        {
