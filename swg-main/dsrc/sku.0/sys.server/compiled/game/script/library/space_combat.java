...
public class space_combat extends script.base_script
{
    ...
    public static void createLoot(obj_id objDefender, obj_id objAttacker) throws InterruptedException
    {
        ...
        String strPilotType = getStringObjVar(objDefender, "ship.pilotType");
        if ((strPilotType != null) && (strTable != null) && strTable.endsWith("_tier1"))
        {
            int tierIndex = strPilotType.lastIndexOf("_tier");
            if (tierIndex > -1)
            {
                String tierValue = strPilotType.substring(tierIndex + 5);
                if (tierValue.equals("2") || tierValue.equals("3"))
                {
                    strTable = strTable.substring(0, strTable.length() - 1) + tierValue;
                }
            }
        }
    ...
    public static obj_id getClosestSpaceStation(obj_id objShip) throws InterruptedException
    {
        ...
        {
            LOG("space_combat", "getClosestSpaceStation: unable to locate quest manager '" + space_quest.QUEST_MANAGER + "'"
                    + " - " + err);
            return null;
        }
        if (!isIdValid(objQuestManager))
        {
            LOG("space_combat", "getClosestSpaceStation: quest manager '" + space_quest.QUEST_MANAGER + "' is not valid");
            return null;
        }
        location locTest = getLocation(objShip);
