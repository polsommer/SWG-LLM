...
public static obj_id callObiwan(obj_id player, obj_id landmark, boolean timeout, int appearanceComment) throws InterruptedException {
    debugLogging("callObiwan", "Entered method.");
    
    if (!isIdValid(player)) {
        debugLogging("callObiwan", "Invalid player ID.");
        return null;
    }

    float distance = 6.0f;
    if (isIdValid(landmark)) {
        distance += getDistance(player, landmark);
        debugLogging("callObiwan", "Landmark location is valid. Adjusted distance: " + distance);
    }

    if (timeout && distance < 11.0f) {
        distance = 11.0f;
    }

    location spawnLoc = utils.findLocInFrontOfTarget(player, distance);
    if (spawnLoc == null) {
        debugLogging("callObiwan", "Spawn location calculation failed. Using player's current location.");
        spawnLoc = getLocation(player);
    }

    obj_id obiwan = create.object("som_kenobi_obi_wan_glowie", spawnLoc);
    if (!isIdValid(obiwan)) {
        debugLogging("callObiwan", "Failed to create Obi-Wan object.");
        return null;
    }
    ...
    debugLogging("callObiwan", "Successfully spawned Obi-Wan.");
    return obiwan;
}
public static void spawnContents(obj_id dungeon, String spawnType, int numberOfCopies) throws InterruptedException {
    debugLogging("spawnContents", "Entered method. SpawnType: " + spawnType);

    if (!isIdValid(dungeon)) {
        debugLogging("spawnContents", "Invalid dungeon ID. Exiting.");
        return;
    }
    ...
    if (spawnTypeColumnData.length == 0) {
        debugLogging("spawnContents", "No data found in event table.");
        return;
    }
    ...
    for (int i = 0; i < spawnTypeColumnData.length; i++) {
        ...
        debugLogging("spawnContents", "Processing row: " + i);

        int locx = dict.getInt("locx");
        int locy = dict.getInt("locy");
        int locz = dict.getInt("locz");

        location dungeonLoc = getLocation(dungeon);
        obj_id cell = getCellId(dungeon, "mainroom");

        location spawnLoc = new location(locx, locy, locz, dungeonLoc.area, cell);
        ...
        if (dict.getString("dataType").equals("setpiece")) {
            debugLogging("spawnContents", "Attempting to spawn setpiece: " + object);
            obj_id item = createObjectInCell(object, dungeon, "mainroom", spawnLoc);

            if (!isIdValid(item)) {
                debugLogging("spawnContents", "Failed to create setpiece: " + object);
                continue;
            }
            ...
            debugLogging("spawnContents", "Successfully created setpiece: " + object);
        } 
        else if (!dict.getString("dataType").equals("location") && !dict.getString("dataType").equals("forcePowerAttack")) {
            debugLogging("spawnContents", "Attempting to spawn creature: " + object);
            ...
            for (int j = 0; j < numberOfCopies; j++) {
                ...
                if (!isIdValid(creature)) {
                    debugLogging("spawnContents", "Failed to create creature: " + object);
                    continue;
                }
                ...
                if (players.length > 0 && creatureName.startsWith("som_kenobi_finale_minion")) {
                    addHate(creature, players[0], 1000.0f);
                }

                utils.setScriptVar(creature, "dungeon", dungeon);
                debugLogging("spawnContents", "Successfully created creature: " + object);
            }
        }
    }
}
