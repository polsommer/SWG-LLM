...
public class pet_lib extends script.base_script
{
    ...
    public static final int MAX_COMBAT_PETS = 2;
    public static final int MAX_NPC_PETS = MAX_COMBAT_PETS;
    public static final int MAX_DROID_PETS = MAX_COMBAT_PETS;
    ...
    public static boolean isCombatPetType(int petType) throws InterruptedException
    {
        return petType == PET_TYPE_NON_AGGRO || petType == PET_TYPE_AGGRO || petType == PET_TYPE_NPC || petType == PET_TYPE_DROID;
    }
    private static int getCallableCountExcluding(obj_id master, int callableType, obj_id petToIgnore) throws InterruptedException
    {
        obj_id[] callables = callable.getCallablesByType(master, callableType);
        if (callables == null || callables.length == 0)
        {
            return 0;
        }
        int count = callables.length;
        if (isIdValid(petToIgnore))
        {
            for (obj_id callableId : callables)
            {
                if (callableId == petToIgnore)
                {
                    count--;
                    break;
                }
    ...
    {
        if (isCombatPetType(petType))
        {
            return MAX_COMBAT_PETS;
        }
    ...
    public static int countPetsOfType(obj_id master, int petType) throws InterruptedException
    {
        if (isCombatPetType(petType))
        {
            return callable.getCallableCount(master, callable.CALLABLE_TYPE_COMBAT_PET);
        }
        else if (petType == PET_TYPE_FAMILIAR)
        {
            return callable.getCallableCount(master, callable.CALLABLE_TYPE_FAMILIAR);
        }
        else if (petType == PET_TYPE_MOUNT)
        {
            return callable.getCallableCount(master, callable.CALLABLE_TYPE_RIDEABLE);
        }
        return 0;
    }
    ...
    public static boolean hasMaxPets(obj_id master, int petType, obj_id pet) throws InterruptedException
    {
        if (isCombatPetType(petType))
        {
            return getCallableCountExcluding(master, callable.CALLABLE_TYPE_COMBAT_PET, pet) >= MAX_COMBAT_PETS;
        }
        else if (petType == PET_TYPE_FAMILIAR)
        {
            return getCallableCountExcluding(master, callable.CALLABLE_TYPE_FAMILIAR, pet) >= MAX_FAMILIAR_PETS;
        }
        else if (petType == PET_TYPE_MOUNT)
        {
            return getCallableCountExcluding(master, callable.CALLABLE_TYPE_RIDEABLE, pet) >= MAX_MOUNT_PETS;
        }
        return false;
    }
    ...
    public static void removeFromList(obj_id master, obj_id pet) throws InterruptedException
    {
        ...
        callable.removeCallable(master, pet, callable.getCallableType(pet));
    }
    public static void addToPetList(obj_id master, obj_id pet) throws InterruptedException
    {
        ...
        int petType = getPetType(pet);
        int callableType = callable.getCallableType(pet);
        if (hasMaxPets(master, petType, pet))
        {
            obj_id[] activePets = callable.getCallablesByType(master, callableType);
            if (activePets != null && activePets.length > 0)
            {
                for (obj_id activePet : activePets)
                {
                    if (activePet == pet)
                    {
                        continue;
                    }
                    callable.storeCallable(master, activePet);
                    if (!hasMaxPets(master, petType, pet))
                    {
                        break;
                    }
        ...
        callable.setCallable(master, pet, callableType);
    }
