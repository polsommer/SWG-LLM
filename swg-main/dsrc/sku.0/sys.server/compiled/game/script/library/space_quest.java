...
public class space_quest extends script.base_script
{
    ...
    public static final String EVENT_MODIFIER_TYPE = "event.modifierType";
    public static final String EVENT_MODIFIER_VALUE = "event.modifierValue";
    public static final String EVENT_MODIFIER_FACTION = "event.modifierFaction";
    ...
    public static boolean flagQuestGroup(obj_id missionObj, int flag) throws InterruptedException
    {
        ...
        boolean success = (flag == QUEST_WON);
        if (group.isGrouped(owner))
        {
            obj_id gid = getGroupObject(owner);
            obj_id[] members = space_utils.getSpaceGroupMemberIds(gid);
            if (members != null && members.length >= 0)
            {
                for (obj_id member : members) {
                    if (isOnGroupQuest(member, questType, questName)) {
                        ...
                        if (member != owner)
                        {
                            space_dynamic_content.completeMission(member, null, success);
                        }
                    }
                }
            }
        ...
        if (!success)
        {
            space_dynamic_content.completeMission(owner, null, false);
        }
        return true;
    }
    ...
    public static boolean setQuestWon(obj_id player, obj_id missionObj, boolean wsplit) throws InterruptedException
    {
        ...
        space_dynamic_content.completeMission(player, null, true);
    ...
    public static boolean setQuestAborted(obj_id player, obj_id missionObj) throws InterruptedException
    {
        ...
        space_dynamic_content.completeMission(player, null, false);
        return true;
    }
    ...
    public static boolean _setupQuest(obj_id player, String questType, String questName, String qTable, boolean duty) throws InterruptedException
    {
        ...
        dictionary activeMission = space_dynamic_content.getActiveMission(player);
        if (activeMission != null)
        {
            String modifierType = activeMission.getString("eventModifierType");
            float modifierValue = activeMission.getFloat("eventModifierValue");
            if (modifierType != null && modifierType.length() > 0)
            {
                setObjVar(missionObj, EVENT_MODIFIER_TYPE, modifierType);
                setObjVar(missionObj, EVENT_MODIFIER_VALUE, modifierValue);
            }
            String missionFaction = activeMission.getString("faction");
            if (missionFaction != null && missionFaction.length() > 0)
            {
                setObjVar(missionObj, EVENT_MODIFIER_FACTION, missionFaction);
            }
        }
    ...
    public static int applyEventObjectiveModifier(obj_id quest, int baseCount) throws InterruptedException
    {
        if (!isIdValid(quest) || baseCount <= 0)
        {
            return baseCount;
        }
        String modifierType = getStringObjVar(quest, EVENT_MODIFIER_TYPE);
        float modifierValue = getFloatObjVar(quest, EVENT_MODIFIER_VALUE);
        if (modifierType == null || modifierType.length() == 0 || modifierValue <= 0.0f)
        {
            return baseCount;
        }
        int adjusted = baseCount;
        if ("extra_objectives".equals(modifierType))
        {
            adjusted = baseCount + Math.round(modifierValue);
        }
        else if ("reward_multiplier".equals(modifierType))
        {
            adjusted = Math.round(baseCount * modifierValue);
        }
        else if ("faction_bonus_imperial".equals(modifierType) || "faction_bonus_rebel".equals(modifierType))
        {
            String missionFaction = getStringObjVar(quest, EVENT_MODIFIER_FACTION);
            String expectedFaction = modifierType.endsWith("imperial") ? "imperial" : "rebel";
            if (missionFaction != null && missionFaction.equalsIgnoreCase(expectedFaction))
            {
                adjusted = Math.round(baseCount * (1.0f + (modifierValue / 100.0f)));
            }
        }
        if (adjusted < baseCount)
        {
            adjusted = baseCount;
        }
        return adjusted;
