...
public class utils extends script.base_script
{
    ...
    public static vector[] getVectorArrayScriptVar(obj_id target, String path) throws InterruptedException
    {
        return target.getScriptVars().getVectorArray(path);
    }
    ...
    public static Vector getVectorScriptVar(obj_id target, String path) throws InterruptedException
    {
        if (!isIdValid(target) || path == null || path.equals(""))
        {
            return null;
        }
        obj_id[] objIds = getObjIdArrayScriptVar(target, path);
        if (objIds != null)
        {
            Vector result = new Vector(objIds.length);
        ...
        String[] strings = getStringArrayScriptVar(target, path);
        if (strings != null)
        {
        ...
        int[] ints = getIntArrayScriptVar(target, path);
        if (ints != null)
        {
            Vector result = new Vector(ints.length);
            for (int value : ints)
            {
        ...
        boolean[] bools = getBooleanArrayScriptVar(target, path);
        if (bools != null)
        {
            Vector result = new Vector(bools.length);
        ...
        float[] floats = getFloatArrayScriptVar(target, path);
        if (floats != null)
        {
        ...
        location[] locations = getLocationArrayScriptVar(target, path);
        if (locations != null)
        {
            Vector result = new Vector(locations.length);
        ...
        transform[] transforms = getTransformArrayScriptVar(target, path);
        if (transforms != null)
        {
            Vector result = new Vector(transforms.length);
            for (transform value : transforms)
            {
                result.add(value);
            }
            return result;
        }
        vector[] vectors = getVectorArrayScriptVar(target, path);
        if (vectors != null)
        {
