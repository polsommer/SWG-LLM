...
public class player_structure extends script.base_script
{
    ...
    public static final String VAR_PLACEMENT_PREVIEW = "player_structure.placement";
    public static final String VAR_PLACEMENT_PREVIEW_TEMPLATE = VAR_PLACEMENT_PREVIEW + ".previewTemplate";
    public static final String VAR_PLACEMENT_PREVIEW_FOOTPRINT = VAR_PLACEMENT_PREVIEW + ".footprintTemplate";
    ...
    public static final int STORAGE_AMOUNT_DECREASE_UNIT = 150;
   ...
   public static obj_id createPlayerStructure(String template, obj_id owner, location loc, int rotation, dictionary deed_info) throws InterruptedException {
    ...
    if (template == null || !isIdValid(owner) || loc == null) {
        return null;
    }
    ...
    // Set rotation and persist the object
    ...
    // Trigger structure creation process with parameters
    dictionary outparams = new dictionary();
    outparams.put("rotation", rotation);
    messageTo(structure, "createStructureObjects", outparams, 10.0f, false);

    // Attach necessary scripts if the structure is not a building or civic
    if (!isBuilding(structure) && !isCivic(structure)) {
        attachScript(structure, SCRIPT_TERMINAL_STRUCTURE);
    }

    // Notify the owner about construction completion
    dictionary params = new dictionary();
    params.put("structure_name", getNameStringId(structure));
    messageTo(owner, "OnConstructionComplete", params, 30.0f, true);

    // Log the construction activity for customer service
    CustomerServiceLog(
        "playerStructure",
        "Structure " + structure + " (" + getName(structure) + ") placed by " + owner + 
        " at location " + loc
    );

    // Handle player ejection range if defined
    ...
    public static boolean canPlaceStructure(obj_id player, String template, location loc, obj_id deed) throws InterruptedException
    {
        ...
        if (isIdValid(deed))
        {
            if (hasObjVar(deed, VAR_DEED_SCENE))
            {
                ...
                if (!match)
                {
                    if (scene.equals("mustafar") && (template.equals("object/building/player/player_house_tcg_vip_bunker.iff") || template.equals("object/building/player/player_house_tcg_commando_bunker.iff")))
                    {
                        match = true;
                    }
                }
    ...
    public static boolean tryEnterPlacementMode(obj_id deedOrScd, obj_id player) throws InterruptedException
    {
        ...
        if (previewTemplate == null || previewTemplate.equals(""))
        {
            previewTemplate = fp_template;
        }
        utils.setScriptVar(deedOrScd, VAR_PLACEMENT_PREVIEW_TEMPLATE, previewTemplate);
        utils.setScriptVar(deedOrScd, VAR_PLACEMENT_PREVIEW_FOOTPRINT, fp_template);
        if (!enterClientStructurePlacementMode(player, deedOrScd, previewTemplate))
        {
            utils.removeScriptVarTree(deedOrScd, VAR_PLACEMENT_PREVIEW);
            return false;
        }
        return true;
    }
    ...
    public static boolean validSafeStorageRemoval(obj_id structure, int decreaseAmount) throws InterruptedException
    {
        ...
        return player_structure.getStructureNumItems(structure) <= projectedAmount;
    }
    ...
    public static boolean decrementStorageAmount(obj_id player, obj_id structure, obj_id terminal, int selectedRedeedTypeIndex) throws InterruptedException
    {
        if (isIdValid(structure) && isIdValid(terminal))
        {
            ...
            if (utils.hasScriptVar(terminal, "storageTypeNamesRef"))
            {
                ...
                if (storageTypeNamesArrayRef != null && storageTypeNamesArrayRef.length > 0 && selectedRedeedTypeIndex >= 0 && selectedRedeedTypeIndex < storageTypeNamesArrayRef.length)
                {
            ...
            int totalIncreasedStorage = getTotalIncreasedStorageAmountStructure(structure);
            if (decreaseAmount > totalIncreasedStorage)
            {
                decreaseAmount = STORAGE_AMOUNT_DECREASE_UNIT;
            }
            int newStrucStorage = totalIncreasedStorage - decreaseAmount;
    ...
    public static boolean handleNpcStructurePlaced(obj_id structure, dictionary params) throws InterruptedException
    {
        obj_id owner = params != null ? params.getObjId("owner") : null;
        if (!isIdValid(structure) || !isIdValid(owner))
        {
            return false;
        }
        if (!exists(structure))
        ...
        if (!ai_lib.isNpc(owner))
        {
            writeNpcStructureOutcome(owner, structure, false, "owner_not_npc");
            return false;
        }

        int now = getGameTime();
        int cleanupSeconds = 3600;
        int maintenancePool = Math.max(1, getIntObjVar(structure, "player_structure.maintenance_pool"));
        setObjVar(structure, "player_structure.maintenance_pool", maintenancePool);
        setObjVar(structure, "npc.simProfile.structure.simulated", 1);
        setObjVar(structure, "npc.simProfile.structure.owner", owner);
        setObjVar(structure, "npc.simProfile.structure.placedAt", now);
        setObjVar(structure, "npc.simProfile.structure.cleanupAt", now + cleanupSeconds);
        setObjVar(structure, "npc.simProfile.structure.cleanupReason", "simulated_npc_placement");

        writeNpcStructureOutcome(owner, structure, true, "placement_persisted");

        dictionary cleanup = new dictionary();
        cleanup.put("source", "adaptive_npc");
        cleanup.put("simulated", 1);
        messageTo(structure, "handleDestroyStructure", cleanup, ((float)cleanupSeconds), false);
        return true;
    }

    private static void writeNpcStructureOutcome(obj_id owner, obj_id structure, boolean success, String detail) throws InterruptedException
    {
        if (!isIdValid(owner))
        {
            return;
        ...
        int now = getGameTime();
        setObjVar(owner, "npc.simProfile.lastSystem", "structure");
        setObjVar(owner, "npc.simProfile.lastAction", "handleNpcStructurePlaced");
        setObjVar(owner, "npc.simProfile.lastSuccess", success ? 1 : 0);
        setObjVar(owner, "npc.simProfile.lastDetail", detail);
        setObjVar(owner, "npc.simProfile.lastTimestamp", now);

        setObjVar(owner, "npc.simProfile.structure.lastPlaced", structure);
        setObjVar(owner, "npc.simProfile.structure.lastSuccess", success ? 1 : 0);
        setObjVar(owner, "npc.simProfile.structure.lastDetail", detail);
        setObjVar(owner, "npc.simProfile.structure.lastTimestamp", now);

        utils.setScriptVar(owner, "npc.simProfile.lastSystem", "structure");
        utils.setScriptVar(owner, "npc.simProfile.lastAction", "handleNpcStructurePlaced");
        utils.setScriptVar(owner, "npc.simProfile.lastSuccess", success ? 1 : 0);
        utils.setScriptVar(owner, "npc.simProfile.lastDetail", detail);
        utils.setScriptVar(owner, "npc.simProfile.lastTimestamp", now);
        utils.setScriptVar(owner, "npc.simProfile.structure.lastSuccess", success ? 1 : 0);
        utils.setScriptVar(owner, "npc.simProfile.structure.lastDetail", detail);
        utils.setScriptVar(owner, "npc.simProfile.structure.lastTimestamp", now);
    }
