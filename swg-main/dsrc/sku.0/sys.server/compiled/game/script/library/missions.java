...
public class missions extends script.base_script
{
    ...
    public static float alterMissionPayoutDivisorDaily(obj_id player, float divisor) throws InterruptedException
    {
        ...
        {
            int excessMissions = missionsCompleted - DAILY_MISSION_CASH_REWARD;
            divisor += excessMissions;
        }
        return divisor;
    }
    ...
    public static boolean isEligibleNpcMission(obj_id npc, obj_id missionData) throws InterruptedException
    {
        if (!isIdValid(npc) || !exists(npc) || !isIdValid(missionData) || !exists(missionData))
        {
            return false;
        }
        if (getMissionStatus(missionData) <= 0)
        {
            return false;
        }
        if (hasObjVar(missionData, "intState"))
        {
            int missionState = getIntObjVar(missionData, "intState");
            if (missionState == STATE_MISSION_COMPLETE || missionState == STATE_DEAD)
            {
                return false;
            }
        }
    ...
    public static obj_id selectEligibleNpcMission(obj_id npc) throws InterruptedException
    {
        if (!isIdValid(npc) || !exists(npc))
        {
            return obj_id.NULL_ID;
        }
        obj_id[] missionObjects = getMissionObjects(npc);
        if (missionObjects == null || missionObjects.length == 0)
        {
            return obj_id.NULL_ID;
        }
        obj_id selected = obj_id.NULL_ID;
        int bestReward = -1;
        for (int i = 0; i < missionObjects.length; i++)
        {
            obj_id missionData = missionObjects[i];
            if (!isEligibleNpcMission(npc, missionData))
            {
                continue;
            }
            int reward = Math.max(0, getMissionReward(missionData));
            if (!isIdValid(selected) || reward > bestReward)
            {
                selected = missionData;
                bestReward = reward;
            }
        }
        return selected;
