...
public class ai_lib extends script.base_script
{
    ...
    public static final String MESSAGE_MODERN_AMBIENT = "handlePerformAmbientLife";
    public static final String SCRIPTVAR_MODERN_AMBIENT_ENABLED = "ai.modernAmbient.enabled";
    public static final String SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT = "ai.modernAmbient.lastChat";
    public static final float MODERN_AMBIENT_MIN_DELAY = 12.0f;
    public static final float MODERN_AMBIENT_MAX_DELAY = 28.0f;
    public static final float MODERN_AMBIENT_NPC_MIN_DELAY = 10.0f;
    public static final float MODERN_AMBIENT_NPC_MAX_DELAY = 24.0f;
    public static final float MODERN_AMBIENT_FAST_DELAY_MIN = 4.0f;
    public static final float MODERN_AMBIENT_FAST_DELAY_MAX = 10.0f;
    public static final float MODERN_AMBIENT_FAST_DELAY_MIN_NPC = 3.0f;
    public static final float MODERN_AMBIENT_FAST_DELAY_MAX_NPC = 8.0f;
    public static final int MODERN_AMBIENT_CHAT_MIN_INTERVAL = 60;
    public static final float MODERN_AMBIENT_CHAT_THRESHOLD = 0.7f;
    public static final float MODERN_AMBIENT_CHAT_THRESHOLD_NPC = 0.55f;
    public static final float MODERN_AMBIENT_ANIMATION_THRESHOLD = 0.35f;
    public static final float MODERN_AMBIENT_ANIMATION_THRESHOLD_MOB = 0.45f;
    public static final float MODERN_AMBIENT_ANIMATION_THRESHOLD_NPC = 0.25f;
    public static final String[] MODERN_AMBIENT_ANIMATIONS =
    {
        "check_wrist_device",
        "fidget",
        "look_left",
    ...
    public static final String[] MODERN_AMBIENT_NPC_EXTRA_ANIMATIONS =
    {
        "adjust_gear",
        "check_datapad",
        "nod_head_once",
    ...
    public static final String[] MODERN_AMBIENT_NPC_ANIMATIONS = mergeAmbientAnimationSets(MODERN_AMBIENT_ANIMATIONS, MODERN_AMBIENT_NPC_EXTRA_ANIMATIONS);
    public static final String[] MODERN_AMBIENT_CHAT_LINES =
    {
        "Stay sharp. Intel says spice runners are bold tonight.",
        "Systems check complete—commlink, vibroblade, attitude.",
        "Keep your head on a swivel; the next holo-brief could drop any second.",
        "If a lightsaber lights up in here, duck first and ask questions later.",
        "Stars above, could use a Corellian caf and a quiet sector.",
        "Breathe, relax. We're the calm in this cantina storm.",
        "Still can't believe we watched a Jedi carve open that blast door.",
        "These streets hum with trouble—stay chill, stay ready."
    };
    public static final String[] MODERN_AMBIENT_CHAT_MOODS =
    {
        chat.MOOD_ALERT,
        chat.MOOD_CONFIDENT,
        chat.MOOD_CASUAL,
    ...
    public static void enableModernAmbientLife(obj_id npc) throws InterruptedException
    {
        if (!isIdValid(npc))
        {
            return;
        }
        utils.setScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_ENABLED, true);
        if (utils.hasScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT))
        {
            utils.removeScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT);
        }
        scheduleModernAmbientLife(npc, true);
    }
    public static boolean hasModernAmbientLife(obj_id npc) throws InterruptedException
    {
        if (!isIdValid(npc) || !exists(npc))
        {
            return false;
        }
        return utils.hasScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_ENABLED) && utils.getBooleanScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_ENABLED);
    }
    public static void scheduleModernAmbientLife(obj_id npc, boolean preferSoon) throws InterruptedException
    {
        if (!hasModernAmbientLife(npc))
        {
            return;
        }
        if (hasMessageTo(npc, MESSAGE_MODERN_AMBIENT))
        {
            return;
        }
        boolean npcActor = isAmbientNpc(npc);
        float minDelay = npcActor ? MODERN_AMBIENT_NPC_MIN_DELAY : MODERN_AMBIENT_MIN_DELAY;
        float maxDelay = npcActor ? MODERN_AMBIENT_NPC_MAX_DELAY : MODERN_AMBIENT_MAX_DELAY;
        if (preferSoon)
        {
            if (npcActor)
            {
                minDelay = MODERN_AMBIENT_FAST_DELAY_MIN_NPC;
                maxDelay = MODERN_AMBIENT_FAST_DELAY_MAX_NPC;
            }
            else
            {
                minDelay = MODERN_AMBIENT_FAST_DELAY_MIN;
                maxDelay = MODERN_AMBIENT_FAST_DELAY_MAX;
            }
        }
        messageTo(npc, MESSAGE_MODERN_AMBIENT, null, rand(minDelay, maxDelay), false);
    }
    public static void performModernAmbientLife(obj_id npc) throws InterruptedException
    {
        if (!hasModernAmbientLife(npc))
        {
            return;
        }
        if (!npc.isLoaded() || ai_lib.isAiDead(npc))
        {
            scheduleModernAmbientLife(npc, false);
            return;
        }
        if (ai_lib.isInCombat(npc) || getBehavior(npc) > BEHAVIOR_CALM || isInNpcConversation(npc))
        {
            scheduleModernAmbientLife(npc, false);
            return;
        }
        boolean performed = false;
        String[] animationPool = getAmbientAnimationsFor(npc);
        if (animationPool.length > 0)
        {
            float animationThreshold = getAmbientAnimationThreshold(npc);
            if (rand() > animationThreshold)
            {
                String animation = animationPool[rand(0, animationPool.length - 1)];
                doAction(npc, animation);
                performed = true;
            }
        }
        boolean allowChat = isAmbientChatAllowed(npc);
        if (!allowChat && utils.hasScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT))
        {
            utils.removeScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT);
        }
        if (allowChat && MODERN_AMBIENT_CHAT_LINES.length > 0)
        {
            int now = getGameTime();
            int lastChat = now - MODERN_AMBIENT_CHAT_MIN_INTERVAL;
            if (utils.hasScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT))
            {
                lastChat = utils.getIntScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT);
            }
            float chatThreshold = isAmbientNpc(npc) ? MODERN_AMBIENT_CHAT_THRESHOLD_NPC : MODERN_AMBIENT_CHAT_THRESHOLD;
            if ((now - lastChat) >= MODERN_AMBIENT_CHAT_MIN_INTERVAL && (rand() > chatThreshold || !performed))
            {
                String mood = chat.MOOD_NEUTRAL;
                if (MODERN_AMBIENT_CHAT_MOODS.length > 0)
                {
                    mood = MODERN_AMBIENT_CHAT_MOODS[rand(0, MODERN_AMBIENT_CHAT_MOODS.length - 1)];
                }
                chat.setChatMood(npc, mood);
                String line = MODERN_AMBIENT_CHAT_LINES[rand(0, MODERN_AMBIENT_CHAT_LINES.length - 1)];
                chat.chat(npc, line);
                utils.setScriptVar(npc, SCRIPTVAR_MODERN_AMBIENT_LAST_CHAT, now);
                performed = true;
            }
        }
        if (!performed && animationPool.length > 0)
        {
            String fallback = animationPool[rand(0, animationPool.length - 1)];
            doAction(npc, fallback);
        }
        scheduleModernAmbientLife(npc, false);
    }
    public static boolean isAmbientNpc(obj_id npc) throws InterruptedException
    {
        if (!isIdValid(npc))
        {
            return false;
    ...
    public static boolean isAmbientChatAllowed(obj_id npc) throws InterruptedException
    {
        if (!isIdValid(npc))
        {
            return false;
    ...
    public static String[] getAmbientAnimationsFor(obj_id npc) throws InterruptedException
    {
        if (isAmbientNpc(npc))
        {
            return MODERN_AMBIENT_NPC_ANIMATIONS;
        }
        return MODERN_AMBIENT_ANIMATIONS;
    }
    public static float getAmbientAnimationThreshold(obj_id npc) throws InterruptedException
    {
        if (isAmbientNpc(npc))
        {
            return MODERN_AMBIENT_ANIMATION_THRESHOLD_NPC;
        }
        if (isMob(npc))
        {
            return MODERN_AMBIENT_ANIMATION_THRESHOLD_MOB;
        }
        return MODERN_AMBIENT_ANIMATION_THRESHOLD;
    }
    public static String[] mergeAmbientAnimationSets(String[] base, String[] extra)
    {
        if (base == null || base.length == 0)
        {
            return (extra == null) ? new String[0] : extra;
        }
        ...
        {
            combined[index++] = base[i];
        }
        for (int j = 0; j < extra.length; j++)
        {
            combined[index++] = extra[j];
        }
        return combined;
    }
