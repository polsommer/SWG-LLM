...
public class guild extends script.base_script
{
    ...
    public static final String DATATABLE_GUILD_PVE_EVENTS = "datatables/guild/guild_pve_events.tab";
    public static final String VAR_GUILD_PVE_LEDGER = "guild.pveLedger";
    public static final String VAR_GUILD_PVE_OBJECTIVES = "guild.pveObjectives";
    public static final int GUILD_PVE_OBJECTIVE_ROTATION_SECONDS = 60 * 60 * 6;
    public static final int GUILD_PVE_OBJECTIVE_COUNT = 3;
    public static final String VAR_PLAYER_GUILD_PVE_PERKS = "guild.pvePerks";
    public static final String GUILD_PVE_PERK_TYPE_BUFF = "buff";
    public static final String GUILD_PVE_PERK_TYPE_RESOURCE = "resource";
    public static final String GUILD_PVE_PERK_TYPE_COSMETIC = "cosmetic";
    public static final String GUILD_PVE_PERK_TYPE_TITLE = "title";
    ...
    public static void applyGuildPvePerks(obj_id player) throws InterruptedException
    {
        if (!isIdValid(player))
        {
            return;
        ...
        if (guildId <= 0)
        {
            clearGuildPvePerksForPlayer(player);
            return;
        }
        obj_id master = getMasterGuildObject();
        if (!isIdValid(master))
        {
            return;
        }
        String[] allEvents = dataTableGetStringColumn(DATATABLE_GUILD_PVE_EVENTS, "event_id");
        if (allEvents == null)
        {
            return;
        }
        ...
        for (String eventId : allEvents) {
            ...
            String perkRoot = getGuildPveEventRoot(guildId, eventId) + ".perk";
            if (!hasObjVar(master, perkRoot + ".type") && !hasObjVar(master, perkRoot + ".value") && !hasObjVar(master, perkRoot + ".expiresAt"))
            {
                clearGuildPvePerkForPlayer(player, eventId);
                continue;
            }
            int expiresAt = hasObjVar(master, perkRoot + ".expiresAt") ? getIntObjVar(master, perkRoot + ".expiresAt") : 0;
            if (expiresAt > 0 && now >= expiresAt)
            {
                clearGuildPvePerk(master, guildId, eventId);
                clearGuildPvePerkForPlayer(player, eventId);
                continue;
            }
            String rewardType = hasObjVar(master, perkRoot + ".type") ? getStringObjVar(master, perkRoot + ".type") : "";
            int rewardValue = hasObjVar(master, perkRoot + ".value") ? getIntObjVar(master, perkRoot + ".value") : 0;
            applyGuildPvePerkToPlayer(player, eventId, rewardType, rewardValue, expiresAt);
        }
    }
    private static void clearGuildPvePerksForPlayer(obj_id player) throws InterruptedException
    {
        if (!isIdValid(player))
        {
            return;
        }
        removeObjVar(player, VAR_PLAYER_GUILD_PVE_PERKS);
    }
    private static void clearGuildPvePerkForPlayer(obj_id player, String eventId) throws InterruptedException
    {
        if (!isIdValid(player) || eventId == null || eventId.length() == 0)
        {
            return;
        }
        removeObjVar(player, VAR_PLAYER_GUILD_PVE_PERKS + "." + eventId);
    }
    private static void clearGuildPvePerk(obj_id master, int guildId, String eventId) throws InterruptedException
    {
        if (!isIdValid(master) || guildId <= 0 || eventId == null || eventId.length() == 0)
        {
            return;
        }
        String perkRoot = getGuildPveEventRoot(guildId, eventId) + ".perk";
        removeObjVar(master, perkRoot + ".type");
        removeObjVar(master, perkRoot + ".value");
        removeObjVar(master, perkRoot + ".expiresAt");
    }
    private static void applyGuildPvePerkToPlayer(obj_id player, String eventId, String rewardType, int rewardValue, int expiresAt) throws InterruptedException
    {
        if (!isIdValid(player) || rewardType == null || rewardType.length() == 0)
        {
            return;
        }
        int remaining = expiresAt > 0 ? expiresAt - getGameTime() : 0;
        if (expiresAt > 0 && remaining <= 0)
        {
            clearGuildPvePerkForPlayer(player, eventId);
            return;
        }
        String playerPerkRoot = VAR_PLAYER_GUILD_PVE_PERKS + "." + eventId;
        setObjVar(player, playerPerkRoot + ".type", rewardType);
        setObjVar(player, playerPerkRoot + ".value", rewardValue);
        if (expiresAt > 0)
        {
            setObjVar(player, playerPerkRoot + ".expiresAt", expiresAt);
        }
        if (rewardType.equals(GUILD_PVE_PERK_TYPE_BUFF))
        {
            String buffName = getGuildPvePerkBuffName(rewardValue);
            if (buffName != null && buffName.length() > 0 && !buff.hasBuff(player, buffName))
            {
                if (expiresAt > 0)
                {
                    buff.applyBuff(player, buffName, remaining);
                }
                else 
                {
        ...
        else if (rewardType.equals(GUILD_PVE_PERK_TYPE_RESOURCE))
        {
            addSkillModModifier(player, "guild_pve_resource_" + eventId, "creature_harvesting", rewardValue, expiresAt > 0 ? remaining : -1, false, true);
        }
        else if (rewardType.equals(GUILD_PVE_PERK_TYPE_COSMETIC))
        {
            setObjVar(player, playerPerkRoot + ".cosmeticActive", 1);
        }
        else if (rewardType.equals(GUILD_PVE_PERK_TYPE_TITLE))
        {
            setObjVar(player, playerPerkRoot + ".titleActive", 1);
        }
    }
    private static String getGuildPvePerkBuffName(int rewardValue) throws InterruptedException
    {
        switch (rewardValue)
        {
            case 1:
                return "buildabuff_inspiration";
            default:
                return "buildabuff_inspiration";
        }
    }
    public static void showGuildOperationsBoard(obj_id player) throws InterruptedException
    {
        if (!isIdValid(player))
        {
            return;
        ...
        if (guildId <= 0)
        {
            sendSystemMessage(player, "You are not currently in a guild.", "");
            return;
        }
        String guildName = guildGetName(guildId);
        String guildAbbrev = guildGetAbbrev(guildId);
        StringBuilder board = new StringBuilder();
        board.append("Guild Operations Board\n");
        if (guildName != null && guildName.length() > 0)
        {
            board.append("Guild: ").append(guildName);
            if (guildAbbrev != null && guildAbbrev.length() > 0)
            {
                board.append(" [").append(guildAbbrev).append("]");
            }
            board.append("\n");
        }
        board.append("\nCurrent PVE Objectives:\n");
        String[] activeObjectives = getActiveGuildPveObjectives();
        if (activeObjectives == null || activeObjectives.length == 0)
        {
            board.append(" - No active objectives at this time.\n");
        }
        else 
        {
            for (String eventId : activeObjectives) {
                String displayName = getGuildPveEventDisplayName(eventId);
                String description = getGuildPveEventDescription(eventId);
                int target = getGuildPveEventTarget(eventId);
                int progress = getGuildPveEventProgress(guildId, eventId);
                String window = getGuildPveEventWindow(eventId);
                if (displayName == null || displayName.length() == 0)
                {
                    displayName = eventId;
                }
                board.append(" - ").append(displayName);
                if (eventId != null && eventId.length() > 0)
                {
                    board.append(" (").append(eventId).append(")");
                }
                board.append(": ").append(progress).append("/").append(target);
                if (description != null && description.length() > 0)
                {
                    board.append(" - ").append(description);
                }
                if (window != null && window.length() > 0)
                {
                    board.append(" [Window: ").append(window).append("]");
                }
                board.append("\n");
            }
        }
        ...
        if (isIdValid(master))
        {
            ...
            if (allEvents != null)
            {
                ...
                for (String eventId : allEvents) {
                    ...
                    board.append(" - ").append(displayName).append(": ").append(rewardType).append(" ").append(rewardValue);
                    if (expiresAt > 0)
                    {
                        int remaining = expiresAt - now;
                        if (remaining > 0)
                        {
                            board.append(" (").append(utils.formatTimeVerbose(remaining)).append(" remaining)");
                        }
                    }
                    board.append("\n");
                    activePerks++;
        ...
        if (activePerks == 0)
        {
            board.append(" - No active perks at this time.\n");
        }
        ...
        {
            for (String eventId : allEvents) {
                if (eventId == null || eventId.length() == 0 || isGuildPveObjectiveActive(activeObjectives, eventId))
                {
                    continue;
                }
                String displayName = getGuildPveEventDisplayName(eventId);
        ...
        if (listed == 0)
        {
            board.append(" - No upcoming windows configured.\n");
        }
        sui.msgbox(player, player, board.toString(), sui.OK_ONLY, "Guild Operations Board", sui.MSG_NORMAL, "onGuildOperationsBoardResponse");
    }
    public static void recordGuildPveContribution(obj_id player, String eventId, int amount, String source) throws InterruptedException
    {
        if (!isIdValid(player) || eventId == null || eventId.length() == 0 || amount <= 0)
        {
            return;
        }
        int guildId = getGuildId(player);
        ...
        String eventRoot = getGuildPveEventRoot(guildId, eventId);
        int current = 0;
        if (hasObjVar(master, eventRoot + ".points"))
        {
            current = getIntObjVar(master, eventRoot + ".points");
        }
        setObjVar(master, eventRoot + ".points", current + amount);
        setObjVar(master, eventRoot + ".lastUpdate", getGameTime());
        setObjVar(master, eventRoot + ".lastContributor", player);
        if (source != null && source.length() > 0)
        {
            setObjVar(master, eventRoot + ".lastSource", source);
        }
        evaluateGuildPveObjective(master, guildId, eventId);
    }
    public static String[] getActiveGuildPveObjectives() throws InterruptedException
    {
        obj_id master = getMasterGuildObject();
        if (!isIdValid(master))
        {
        ...
        int now = getGameTime();
        int nextRotation = 0;
        if (hasObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".nextRotation"))
        {
            nextRotation = getIntObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".nextRotation");
        }
        String[] eventIds = getStringArrayObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".eventIds");
        if (eventIds == null || eventIds.length == 0 || nextRotation == 0 || now >= nextRotation)
        {
            rotateGuildPveObjectives(master, now);
            eventIds = getStringArrayObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".eventIds");
        }
        if (eventIds == null)
        {
            return new String[0];
    ...
    public static int getGuildPveEventProgress(int guildId, String eventId) throws InterruptedException
    {
        if (guildId <= 0 || eventId == null || eventId.length() == 0)
        {
            return 0;
        }
        obj_id master = getMasterGuildObject();
        ...
        if (hasObjVar(master, eventRoot + ".points"))
        {
            return getIntObjVar(master, eventRoot + ".points");
        }
        return 0;
    }
    public static int getGuildPveEventTarget(String eventId) throws InterruptedException
    {
        if (eventId == null || eventId.length() == 0)
        {
            return 0;
        }
        return dataTableGetInt(DATATABLE_GUILD_PVE_EVENTS, eventId, "target");
    }
    public static String getGuildPveEventDisplayName(String eventId) throws InterruptedException
    {
        if (eventId == null || eventId.length() == 0)
        {
            return "";
        }
        return dataTableGetString(DATATABLE_GUILD_PVE_EVENTS, eventId, "display_name");
    }
    public static String getGuildPveEventDescription(String eventId) throws InterruptedException
    {
        if (eventId == null || eventId.length() == 0)
        {
            return "";
        }
        return dataTableGetString(DATATABLE_GUILD_PVE_EVENTS, eventId, "description");
    }
    public static String getGuildPveEventWindow(String eventId) throws InterruptedException
    {
        if (eventId == null || eventId.length() == 0)
        {
            return "";
        }
        String start = dataTableGetString(DATATABLE_GUILD_PVE_EVENTS, eventId, "window_start");
        String end = dataTableGetString(DATATABLE_GUILD_PVE_EVENTS, eventId, "window_end");
    ...
    private static void rotateGuildPveObjectives(obj_id master, int now) throws InterruptedException
    {
        String[] allEventIds = dataTableGetStringColumn(DATATABLE_GUILD_PVE_EVENTS, "event_id");
        if (allEventIds == null || allEventIds.length == 0)
        {
            setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".eventIds", new String[0]);
            setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".nextRotation", now + GUILD_PVE_OBJECTIVE_ROTATION_SECONDS);
            setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".rotationId", now);
            return;
        }
        Vector eventPool = new Vector(Arrays.asList(allEventIds));
        int count = Math.min(GUILD_PVE_OBJECTIVE_COUNT, eventPool.size());
        String[] selected = new String[count];
        for (int i = 0; i < count; i++)
        {
            int pick = rand(0, eventPool.size() - 1);
            selected[i] = (String)eventPool.elementAt(pick);
            eventPool.removeElementAt(pick);
        }
        setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".eventIds", selected);
        setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".nextRotation", now + GUILD_PVE_OBJECTIVE_ROTATION_SECONDS);
        setObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".rotationId", now);
    }
    private static void evaluateGuildPveObjective(obj_id master, int guildId, String eventId) throws InterruptedException
    {
        if (!isGuildPveObjectiveActive(getActiveGuildPveObjectives(), eventId))
        {
            return;
        }
        int target = getGuildPveEventTarget(eventId);
        ...
        int rotationId = getGuildPveRotationId(master);
        String eventRoot = getGuildPveEventRoot(guildId, eventId);
        int rewardedRotation = 0;
        if (hasObjVar(master, eventRoot + ".rewardedRotation"))
        {
            rewardedRotation = getIntObjVar(master, eventRoot + ".rewardedRotation");
        }
        if (rewardedRotation == rotationId)
        {
            return;
        }
        awardGuildPvePerk(guildId, eventId);
        setObjVar(master, eventRoot + ".rewardedRotation", rotationId);
    }
    private static int getGuildPveRotationId(obj_id master) throws InterruptedException
    {
        if (hasObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".rotationId"))
        {
            return getIntObjVar(master, VAR_GUILD_PVE_OBJECTIVES + ".rotationId");
        }
        return 0;
    }
    private static void awardGuildPvePerk(int guildId, String eventId) throws InterruptedException
    {
        if (guildId <= 0 || eventId == null || eventId.length() == 0)
        {
            return;
        ...
        String rewardType = dataTableGetString(DATATABLE_GUILD_PVE_EVENTS, eventId, "reward_type");
        int rewardValue = dataTableGetInt(DATATABLE_GUILD_PVE_EVENTS, eventId, "reward_value");
        int duration = dataTableGetInt(DATATABLE_GUILD_PVE_EVENTS, eventId, "perk_duration");
        int expiresAt = duration > 0 ? getGameTime() + duration : 0;
        String perkRoot = getGuildPveEventRoot(guildId, eventId) + ".perk";
        if (rewardType != null && rewardType.length() > 0)
        {
            setObjVar(master, perkRoot + ".type", rewardType);
        }
        setObjVar(master, perkRoot + ".value", rewardValue);
        if (expiresAt > 0)
        {
            setObjVar(master, perkRoot + ".expiresAt", expiresAt);
        }
        String displayName = getGuildPveEventDisplayName(eventId);
        if (displayName == null || displayName.length() == 0)
        {
        ...
        String message = "Guild objective completed: " + displayName + ". Perk awarded (" + rewardType + " " + rewardValue + ").";
        obj_id[] members = getMemberIds(guildId, false, true);
        if (members != null)
        {
            for (obj_id member : members) {
                if (isIdValid(member) && isPlayerConnected(member)) {
                    sendSystemMessage(member, message, "");
                }
            }
        }
    }
    private static String getGuildPveEventRoot(int guildId, String eventId)
    {
        return VAR_GUILD_PVE_LEDGER + "." + guildId + ".events." + eventId;
    }
    private static boolean isGuildPveObjectiveActive(String[] activeObjectives, String eventId)
    {
        if (activeObjectives == null || eventId == null)
        {
            return false;
        }
        for (String active : activeObjectives) {
